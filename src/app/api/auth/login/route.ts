    ...
    // ── Kolom yang diambil dari DB — eksplisit agar tidak miss kolom baru ─────────
    // ✅ Tambah "roles" agar multi-role bisa masuk JWT
    ...
    export async function POST(request: Request) {
        try {
            ...
            if (looksLikePhone) {
                // Coba 3 format nomor secara paralel: 628xxx, raw digits, 08xxx
                ...
                const [r1, r2, r3] = await Promise.all([
                    supabase.from("users").select(USER_SELECT_FIELDS).eq("phone_number", normalized).maybeSingle(),
                    supabase.from("users").select(USER_SELECT_FIELDS).eq("phone_number", rawDigits).maybeSingle(),
                    supabase.from("users").select(USER_SELECT_FIELDS).eq("phone_number", withZero).maybeSingle(),
                ]);
            ...
            // Fallback ke [role] untuk backward compat
            const userRolesArray: string[] =
                Array.isArray(user.roles) && user.roles.length > 0
                    ? user.roles
                    : [user.role];

            // ✅ buildTokenPayload memastikan JWT punya field "role" (primary) & "roles" (array)
            ...
            // Gunakan instance terpisah untuk operasi admin
            ...
            const response = NextResponse.json(
                {
                    success: true,
                    redirect,
                    // ✅ Kirim roles array ke client agar sidebar & pages bisa pakai langsung
                    user: {
                        name: user.name,
                        role: primaryRole,
                        roles: userRolesArray,
