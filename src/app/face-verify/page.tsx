...
function isAttendanceTime(): AttendanceTimeResult {
...
export default function FaceVerifyPage() {
  ...
  const pad = (n: number) => String(n).padStart(2, "0");
  const openAtStr  = `${pad(ATTENDANCE_START_HOUR)}:${pad(ATTENDANCE_START_MIN)}`;
  const closeAtStr = `${pad(ATTENDANCE_END_HOUR)}:${pad(ATTENDANCE_END_MIN)}`;
