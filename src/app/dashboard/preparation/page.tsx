    ...
    (async () => {
      try {
        ...
        try {
          ...
          else setError("Gagal memulai scanner di browser ini. Kamu masih bisa ketik SN manual.");
        }
      }
    })();
 ...
 const addManual = () => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const result = await res.json();
      if (!result.success) {
        setError(result.message || "Gagal membuat penyiapan");
        return;
      }
      onCreated();
      onClose();
...
interface StatusCounts {
  ...
  diproses: number;
  siap_kirim: number;
  menunggu_pengantar: number;
  dikirim: number;
  selesai: number;
  dibatalkan: number;
  total: number;
  unit_menunggu: number;
}
...
export default function PreparationPage() {
  ...
  const STAT_CARDS = [
    { key: "MENUNGGU", label: "Menunggu", value: sc?.menunggu ?? 0, color: "amber", icon: <Clock size={20} /> },
    { key: "DIPROSES", label: "Diproses", value: sc?.diproses ?? 0, color: "blue", icon: <Wrench size={20} /> },
    { key: "DIKIRIM", label: "Dikirim", value: sc?.dikirim ?? 0, color: "violet", icon: <Truck size={20} /> },
    { key: "SELESAI", label: "Selesai", value: sc?.selesai ?? 0, color: "emerald", icon: <CheckCircle2 size={20} /> },
  ];
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <span className="inline-flex items-center gap-1.5 text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-200">
            ...
            <div className="flex items-center gap-2">
                ...
                {soundOn ? (
                  ...
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z M17 14l4-4m0 4l-4-4"
                    />
                  </svg>
                )}
              </button>
              {canCreate && (
                <button
                  ...
                  className="inline-flex items-center gap-2 h-9 px-5 bg-gradient-to-r from-gray-800 to-gray-900 rounded-xl text-sm font-semibold text-white hover:from-gray-900 hover:to-black active:scale-[0.97] transition shadow-lg shadow-gray-800/25"
                >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {STAT_CARDS.map((s) => (
              ...
              >
                ...
                <p className="text-[11px] font-bold uppercase tracking-wide mt-1.5 opacity-80">
                  {s.label}
                </p>
              </button>
            ))}
