...
import { Sun, Cloud, Moon, RefreshCw, Users, Search, UserX, ChevronDown, ChevronUp } from "lucide-react";
    ...
    <DashboardLayout>
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              disabled={refreshing}
              className="flex items-center gap-1.5 text-xs font-semibold text-gray-500 hover:text-gray-700 border border-gray-200 px-4 py-2 rounded-xl bg-white hover:shadow-md transition-all active:scale-95 disabled:opacity-50"
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {SECTIONS.map((s) => (
            <div key={s.key} className={`bg-gradient-to-br ${s.gradient} rounded-2xl border border-gray-100 shadow-sm p-4`}>
          ...
          <div className="bg-gradient-to-br from-emerald-50 to-green-50 rounded-2xl border border-gray-100 shadow-sm p-4">
            <div className="flex items-center justify-between mb-2">
        ...
        {!loading && totalHadir > 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
            ...
            <div className="flex items-center gap-4 mt-2 flex-wrap">
              {SECTIONS.map((s) => (
                <span key={s.key} className="inline-flex items-center gap-1.5 text-[10px] text-gray-500 font-medium">
        ...
        {/* Search — filter cepat lintas 3 kotak */}
        <div className="relative">
          ...
          <input
            ...
            className="w-full h-11 border border-gray-200 rounded-xl pl-10 pr-4 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#1a1a2e]/10 transition-all"
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {SECTIONS.map((section) => {
            const entries = data[section.key].filter((e) => filterMatch(e.name, e.role));
            return (
              <div
                key={section.key}
                className={`bg-gradient-to-br ${section.gradient} rounded-2xl border border-gray-100 shadow-sm overflow-hidden`}
              >
                ...
                <div className="p-3 space-y-2 min-h-[140px] max-h-[420px] overflow-y-auto">
                  {loading ? (
                    ...
                    entries.map((e) => (
                      <div
                        key={e.user_id}
                        className="flex items-center gap-3 bg-white rounded-xl px-3 py-2.5 shadow-sm border border-gray-50 hover:shadow-md transition-shadow"
                      >
        ...
        {/* Belum Tercatat — fitur tambahan, collapsible */}
