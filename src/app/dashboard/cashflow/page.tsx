...
function ExpenseModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
    ...
    const savingLabel = uploadProgress === "uploading" ? "Mengupload foto..." : uploadProgress === "done" ? "Menyimpan..." : saving ? "Menyimpan..." : "Simpan Pengeluaran";
...
function EditExpenseModal({ entry, onClose, onSaved }: { entry: Entry; onClose: () => void; onSaved: () => void }) {
    ...
    const inputCls = "w-full h-10 border border-gray-200 rounded-lg px-3 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-400/30 focus:border-blue-400 transition";
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
            ...
            <div className="relative bg-white w-full sm:max-w-md rounded-2xl shadow-2xl overflow-hidden border border-gray-100">
                ...
                <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center"><IconEdit /></div>
                        <div>
                            <p className="text-sm font-bold text-gray-900">Edit Pengeluaran</p>
                            <p className="text-[11px] text-gray-400">
                                Dibuat oleh <span className="font-semibold">{entry.created_by_user?.name ?? entry.nama}</span> · {fmtTanggal(entry.tanggal)}
                            </p>
                        </div>
                    </div>
                    <button onClick={onClose} className="w-7 h-7 rounded-lg text-gray-400 hover:text-gray-700 hover:bg-gray-100 flex items-center justify-center transition"><IconX /></button>
                ...
                <div className="p-5 space-y-3.5 max-h-[75vh] overflow-y-auto">
                    {entry.photo_url && (
                        <div className="flex items-start gap-2 px-3 py-2.5 rounded-lg bg-blue-50 border border-blue-100 text-[11px] text-blue-700">
                            <IconInfo />
                            <span>Entry ini punya foto bukti. Foto tidak dapat diubah melalui edit.</span>
                        </div>
                    )}
                    <div>
                        <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Metode Pembayaran <span className="text-red-500">*</span></label>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Nominal <span className="text-red-500">*</span></label>
                            <input type="number" value={nominal} onChange={(e) => setNominal(e.target.value)} placeholder="0" className={`${inputCls} font-mono`} autoFocus />
                            {nominal && Number(nominal) > 0 && <p className="text-[11px] text-blue-500 mt-1 font-mono font-semibold">{fmtRupiah(Number(nominal))}</p>}
                        </div>
                        <div>
                            <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Tanggal</label>
                            <input type="date" value={tanggal} onChange={(e) => setTanggal(e.target.value)} className={inputCls} />
                    ...
                    <div>
                        <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Keterangan</label>
                        <textarea value={keterangan} onChange={(e) => setKeterangan(e.target.value)} rows={3} placeholder="Catatan tambahan..." className={`${inputCls.replace("h-10", "")} py-2 resize-none`} />
                    </div>
                    {error && <div className="bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-xs text-red-700">{error}</div>}
                </div>
                <div className="px-5 py-4 border-t border-gray-100 flex gap-3 bg-gray-50/60">
                    <button onClick={onClose} disabled={saving} className="flex-1 h-10 bg-white border border-gray-200 text-gray-600 rounded-lg text-sm font-medium hover:bg-gray-50 transition disabled:opacity-50">Batal</button>
                    <button onClick={submit} disabled={saving} className="flex-1 h-10 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition disabled:opacity-60">
...
export default function CashflowPage() {
    ...
    // Helper conditions
    const isManualOut = (e: Entry) => e.direction === "OUT" && e.source_type === "MANUAL";
    const editable = (e: Entry) => isManualOut(e) && !e.is_audited;
    const deletable = (e: Entry) => isManualOut(e) && !e.is_audited;
    ...
    return (
        <DashboardLayout>
            ...
            {editingEntry && <EditExpenseModal entry={editingEntry} onClose={() => setEditingEntry(null)} onSaved={() => fetchData(true)} />}
            {deletingEntry && (
                <DeleteConfirmModal
                    entry={deletingEntry}
            ...
            <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6 space-y-5">
                ...
                <div className="flex items-center justify-between gap-3 flex-wrap">
                    ...
                    {/* ✅ CTA — OUT: tombol tambah, IN: label info */}
                    {tab === "OUT" ? (
                        <button
                                        ...
                                        <tr key={i} className="animate-pulse">
                                            {Array.from({ length: 10 }).map((__, j) => (
                                                <td key={j} className="px-3.5 py-3.5">
                                                    <div className="h-3 rounded-full bg-gray-100" style={{ width: j === 3 ? 100 : j === 5 ? 80 : 56 }} />
                                                </td>
                                            ))}
                                        </tr>
                                    ))
                                        ...
                                        return (
                                            <tr key={e.id} className="hover:bg-gray-50/40 transition-colors">
                                                ...
                                                <td className="px-3 py-3 max-w-[180px]">
                                                    ...
                                                    {e.photo_url && (
                                                        <a href={e.photo_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 mt-0.5 text-[10px] font-semibold text-blue-600 hover:underline">
                                                            📷 Foto
                                                        </a>
                                                    )}
                                                </td>
                                                ...
                                                {/* ✅ Kolom Aksi — selalu tampil, tombol jelas */}
                                                <td className="px-3 pr-5 py-3 whitespace-nowrap" onClick={(ev) => ev.stopPropagation()}>
                                                    <div className="flex items-center justify-center gap-1">

                                                        {/* OUT MANUAL: Edit + Delete + status Terkunci */}
                                                        {isManual && (
                                                            <>
                                                                {/* Tombol Edit */}
                                                                <button
                                                                    onClick={() => isEditable && setEditingEntry(e)}
                                                                    disabled={!isEditable}
                                                                    className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-semibold border transition
                                                                        ${isEditable
                                                                            ? "text-blue-600 bg-blue-50 border-blue-200 hover:bg-blue-100 cursor-pointer"
                                                                            : "text-gray-300 bg-gray-50 border-gray-100 cursor-not-allowed"
                                                                        }`}
                                                                    title={isEditable ? "Edit entry ini" : "Sudah diaudit — tidak bisa diedit"}
                                                                >
                                                                    <IconEdit />
                                                                    <span>Edit</span>
                                                                </button>
                                                                ...
                                                                {/* Tombol Hapus */}
                                                                <button
                                                                    onClick={() => isDeletable && setDeletingEntry(e)}
                                                                    ...
                                                                    title={isDeletable ? "Hapus entry ini" : "Sudah diaudit — tidak bisa dihapus"}
                                                                >
                                                                    <IconTrash />
                                                                    <span>Hapus</span>
                                                                </button>

                                                                {/* Badge Terkunci jika sudah audit */}
                                                                {e.is_audited && (
                                                                    <span
                                                                        className="inline-flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-semibold text-emerald-600 bg-emerald-50 border border-emerald-200"
                                                                        title="Entry sudah diaudit dan terkunci"
                                                                    >
                                                                        <IconLock /> Terkunci
                                                                    </span>
                                                                )}
                                                        ...
                                                        {/* OUT non-MANUAL (auto-sync TRX/SVC): label keterangan */}
                                                        {e.direction === "OUT" && !isManual && (
                                                            <span className="text-[10px] text-gray-300 italic select-none">Auto-sync</span>
                                                        )}

                                                        {/* IN TRANSACTION / SERVICE: tombol Buka sumber */}
                                                        {isClickable && (
                                                            <button
                                                                onClick={() => openSource(e)}
                                                                className="inline-flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-semibold text-gray-600 bg-gray-50 border border-gray-200 hover:bg-gray-100 hover:text-gray-800 transition cursor-pointer"
                                                                title="Buka di Riwayat Transaksi / Service"
                                                            >
                                                                <IconExternal /> Buka
                                                            </button>
                                                        )}
                                                        ...
                                                        {/* IN MODAL_AWAL: label */}
                                                        {e.direction === "IN" && e.source_type === "MODAL_AWAL" && (
                                                            <span className="text-[10px] text-violet-400 font-semibold italic select-none">Modal Awal</span>
                                                        )}
                                                    </div>
                                                </td>
                                            </tr>
                                ...
                                <div className="flex items-center gap-1">
                                    ...
                                    {(() => {
                                        ...
                                        return pages.map((p, i) =>
                                            p === "..." ? (
                                                <span key={`d-${i}`} className="w-7 h-7 flex items-center justify-center text-[11px] text-gray-400">…</span>
                                            ) : (
