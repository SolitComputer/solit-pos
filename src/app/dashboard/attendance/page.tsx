...
function ScheduleModal({ users, onClose }: { users: UserInfo[]; onClose: () => void }) {
    ...
    const [form, setForm] = useState({
        day_of_week: 1, start_hour: 8, start_minute: 0,
        late_hour: 9, late_minute: 0, end_hour: 17, end_minute: 0, notes: ""
    });

    useEffect(() => {
        if (!uid) return;
        fetch(`/api/attendance/schedule?user_id=${uid}`)
    ...
    const save = async () => {
        setSaving(true);
        try {
            await fetch("/api/attendance/schedule", {
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            <div className="relative bg-white w-full sm:max-w-2xl rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[90dvh] overflow-hidden">
                <div className="bg-blue-600 px-5 py-4 flex items-center justify-between flex-shrink-0">
                    <div><p className="font-bold text-white text-sm">🕐 Jadwal Custom per Hari</p><p className="text-xs text-blue-100 mt-0.5">Atur jam masuk berbeda per hari per karyawan</p></div>
                ...
                <div className="px-5 pt-4 pb-3 border-b border-gray-100 flex-shrink-0">
                    ...
                    <select value={uid} onChange={e => setUid(e.target.value)} className="w-full h-10 border border-gray-200 rounded-xl px-3 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-400/30">
                ...
                <div className="overflow-y-auto flex-1 px-5 py-4 space-y-4">
                    ...
                    <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
                        <p className="text-xs font-bold text-blue-700 mb-3">Tambah / Update Jadwal</p>
                        <div className="grid grid-cols-2 gap-3 mb-3">
                            <div>
                                <label className="text-[10px] font-semibold text-gray-500 mb-1 block">Hari</label>
                                <select value={form.day_of_week} onChange={e => setForm(f => ({ ...f, day_of_week: Number(e.target.value) }))}
                                    className="w-full h-9 border border-gray-200 rounded-lg px-2 text-xs bg-white focus:outline-none">
                                    {DAY_FULL.map((d, i) => <option key={i} value={i}>{d}</option>)}
                                </select>
                            </div>
                            <div>
                                <label className="text-[10px] font-semibold text-gray-500 mb-1 block">Catatan (opsional)</label>
                                <input type="text" value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                                    placeholder="e.g. kelas pagi" className="w-full h-9 border border-gray-200 rounded-lg px-2 text-xs bg-white focus:outline-none" />
                            </div>
                        </div>
                        <div className="grid grid-cols-3 gap-3">
                            {[
                                { label: "Buka Absen", hKey: "start_hour", mKey: "start_minute" },
                                { label: "Batas Tepat", hKey: "late_hour", mKey: "late_minute" },
                                { label: "Tutup Absen", hKey: "end_hour", mKey: "end_minute" },
                            ].map(({ label, hKey, mKey }) => (
                                <div key={hKey}>
                                    <label className="text-[10px] font-semibold text-gray-500 mb-1 block">{label}</label>
                                    <div className="flex gap-1 items-center">
                                        <input type="number" min={0} max={23} value={(form as any)[hKey]}
                                            onChange={e => setForm(f => ({ ...f, [hKey]: Number(e.target.value) }))}
                                            className="w-12 h-9 border border-gray-200 rounded-lg px-1.5 text-xs bg-white focus:outline-none text-center" />
                                        <span className="text-gray-400 text-xs">:</span>
                                        <input type="number" min={0} max={59} value={(form as any)[mKey]}
                                            onChange={e => setForm(f => ({ ...f, [mKey]: Number(e.target.value) }))}
                                            className="w-12 h-9 border border-gray-200 rounded-lg px-1.5 text-xs bg-white focus:outline-none text-center" />
                                    </div>
                                </div>
                            ))}
                        ...
                        <button onClick={save} disabled={saving}
                            className="mt-3 w-full h-9 bg-blue-600 text-white rounded-lg text-xs font-semibold hover:bg-blue-700 transition disabled:opacity-50 flex items-center justify-center gap-2">
                            {saving ? <><div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />Menyimpan...</> : "💾 Simpan Jadwal"}
                        </button>
                    </div>
                    ...
                    {schedules.length > 0 && (
                        <div>
                            <p className="text-xs font-bold text-gray-600 mb-2">Jadwal Tersimpan</p>
                            <div className="space-y-2">
                                {schedules.map(s => (
                                    <div key={s.id} className="bg-white border border-gray-100 rounded-xl px-4 py-3 flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-semibold text-gray-800">{DAY_FULL[s.day_of_week]}</p>
                                        ...
                                        <button onClick={() => del(s.day_of_week)}
                                            className="w-7 h-7 flex items-center justify-center rounded-full text-red-400 hover:bg-red-50 hover:text-red-600 transition text-sm">
                                            ×
                                        </button>
                                    </div>
                                ))}
                    ...
                    {schedules.length === 0 && (
                        <div className="text-center py-6 text-gray-400 text-xs">
                            Belum ada jadwal custom. Menggunakan jam shift default.
                        </div>
                    )}
                </div>
...
function DateScheduleModal({ users, calYear, calMonth, onClose }: {
    users: UserInfo[]; calYear: number; calMonth: number; onClose: () => void;
}) {
    const [uid, setUid] = useState(users[0]?.id ?? "");
    const [schedules, setSchedules] = useState<any[]>([]);
    const [saving, setSaving] = useState(false);
    ...
    // Simpan satu tanggal atau range tanggal
    const save = async () => {
        ...
        try {
            ...
            await Promise.all(dates.map(date =>
                fetch("/api/attendance/date-schedule", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                        user_id: uid,
                        schedule_date: date,
                        start_hour: form.start_hour, start_minute: form.start_minute,
                        late_hour: form.late_hour, late_minute: form.late_minute,
                        end_hour: form.end_hour, end_minute: form.end_minute,
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            <div className="relative bg-white w-full sm:max-w-2xl rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[90dvh] overflow-hidden">
                ...
                <div className="bg-violet-600 px-5 py-4 flex items-center justify-between flex-shrink-0">
                    <div>
                        <p className="font-bold text-white text-sm">📆 Jadwal Tanggal Spesifik</p>
                        <p className="text-xs text-violet-100 mt-0.5">Override jam masuk untuk tanggal / range tertentu</p>
                    </div>
                    <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full text-white/60 hover:text-white hover:bg-white/20 transition">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                ...
                <div className="px-5 pt-4 pb-3 border-b border-gray-100 flex-shrink-0">
                    ...
                    <select value={uid} onChange={e => setUid(e.target.value)}
                        className="w-full h-10 border border-gray-200 rounded-xl px-3 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-violet-400/30">
                        {users.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                    </select>
                </div>
                ...
                <div className="overflow-y-auto flex-1 px-5 py-4 space-y-4">
                    ...
                    <div className="bg-violet-50 border border-violet-100 rounded-xl p-4">
                        <p className="text-xs font-bold text-violet-700 mb-3">Tambah Jadwal Tanggal</p>

                        {/* Range tanggal */}
                        <div className="grid grid-cols-2 gap-3 mb-3">
                            <div>
                                <label className="text-[10px] font-semibold text-gray-500 mb-1 block">Dari Tanggal</label>
                                <input type="date" value={form.from_date}
                                    onChange={e => setForm(f => ({ ...f, from_date: e.target.value }))}
                                    min={`${calYear}-${String(calMonth + 1).padStart(2, "0")}-01`}
                                    max={`${calYear}-${String(calMonth + 1).padStart(2, "0")}-${new Date(calYear, calMonth + 1, 0).getDate()}`}
                                    className="w-full h-9 border border-gray-200 rounded-lg px-2 text-xs bg-white focus:outline-none focus:ring-1 focus:ring-violet-400" />
                            </div>
                            <div>
                                <label className="text-[10px] font-semibold text-gray-500 mb-1 block">Sampai Tanggal <span className="text-gray-300">(opsional)</span></label>
                                <input type="date" value={form.to_date}
                                    onChange={e => setForm(f => ({ ...f, to_date: e.target.value }))}
                                    min={form.from_date || `${calYear}-${String(calMonth + 1).padStart(2, "0")}-01`}
                                    max={`${calYear}-${String(calMonth + 1).padStart(2, "0")}-${new Date(calYear, calMonth + 1, 0).getDate()}`}
                                    className="w-full h-9 border border-gray-200 rounded-lg px-2 text-xs bg-white focus:outline-none focus:ring-1 focus:ring-violet-400" />
                                <p className="text-[9px] text-gray-400 mt-0.5">Kosongkan untuk 1 hari saja</p>
                            </div>
                        </div>
                        ...
                        <input type="text" value={form.notes}
                            ...
                            className="w-full h-9 border border-gray-200 rounded-lg px-3 text-xs bg-white focus:outline-none mb-3" />

                        {/* Preview */}
                        {form.from_date && (
                            <div className="bg-white border border-violet-100 rounded-lg px-3 py-2 mb-3 text-[11px] text-gray-600">
                                <span className="font-semibold text-violet-700">Preview: </span>
                        ...
                        <button onClick={save} disabled={saving || !form.from_date}
                            className="w-full h-9 bg-violet-600 text-white rounded-lg text-xs font-semibold hover:bg-violet-700 transition disabled:opacity-50 flex items-center justify-center gap-2">
                            {saving ? <><div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />Menyimpan...</> : "💾 Simpan Jadwal Tanggal"}
                        </button>
                    </div>
                    ...
                    {schedules.length > 0 ? (
                        <div>
                            <p className="text-xs font-bold text-gray-600 mb-2">Jadwal Tersimpan — {MONTH_NAMES[calMonth]} {calYear}</p>
                            <div className="space-y-1.5">
                                {schedules.map(s => (
                                    <div key={s.id} className="bg-white border border-gray-100 rounded-xl px-4 py-2.5 flex items-center justify-between">
                                        <div>
                                            <p className="text-xs font-semibold text-gray-800">
                                                {new Date(s.schedule_date + "T12:00:00").toLocaleDateString("id-ID", { weekday: "short", day: "numeric", month: "short" })}
                                            </p>
                                        ...
                                        <button onClick={() => del(s.schedule_date)}
                                            className="w-7 h-7 flex items-center justify-center rounded-full text-red-400 hover:bg-red-50 hover:text-red-600 transition text-sm flex-shrink-0">
                                            ×
                                        </button>
                                    </div>
                                ))}
                        ...
                        <div className="text-center py-6 text-gray-400 text-xs">
                            Belum ada jadwal tanggal spesifik untuk {MONTH_NAMES[calMonth]} {calYear}
                        </div>
                    )}
                </div>
        ...
        <DashboardLayout>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <button onClick={() => setShowScheduleModal(true)}
                            className="flex items-center gap-1.5 text-xs font-medium text-blue-600 bg-blue-50 border border-blue-200 px-3 py-2 rounded-xl hover:bg-blue-100 transition">
                            🕐 Jadwal Custom
                        </button>
                        <button onClick={async () => {
                            ...
                            className="flex items-center gap-1.5 text-xs font-medium text-violet-600 bg-violet-50 border border-violet-200 px-3 py-2 rounded-xl hover:bg-violet-100 transition">
                            📆 Jadwal Tanggal
                        </button>
                    </div>
                </div>
                                ...
                                {calDays.map((day, idx) => {
                                    ...
                                    return (
                                        <button key={day} onClick={() => setSelectedDate(p => p === dk ? null : dk)}
                                            ...
                                            className={`relative flex flex-col items-start justify-start p-2 rounded-xl min-h-[72px] transition-all ${isSel ? "bg-[#1a1a2e] shadow-lg ring-2 ring-[#1a1a2e]/30"
                                            ...
                                            {/* Angka hari */}
                                            <span className={`text-sm font-bold leading-none mb-1.5 ${isSel ? "text-white" : isTod ? "text-blue-600" : isUserDayOff ? "text-red-500" : "text-gray-800"
                                                }`}>{day}</span>
            ...
            {showDateScheduleModal && currentUser?.role === "ADMIN" && (
                <DateScheduleModal
                    ...
                    calYear={calYear}
                    calMonth={calMonth}
                    onClose={() => setShowDateScheduleModal(false)}
                />
            )}
