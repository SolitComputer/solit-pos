...
const lbl = "text-[9px] font-semibold text-gray-400 uppercase tracking-widest block mb-1.5";
const primaryBtn = "flex-1 h-9 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-xs font-semibold transition-all active:scale-[0.98] flex items-center justify-center gap-1.5 disabled:opacity-40 disabled:cursor-not-allowed";
const secondaryBtn = "flex-1 h-9 bg-white border border-gray-200 text-gray-600 rounded-lg text-xs font-semibold hover:bg-gray-50 transition-all active:scale-[0.98] flex items-center justify-center";

function ErrorBanner({ msg }: { msg: string }) {
  return <div className="flex items-start gap-2 bg-red-50 border border-red-200 text-red-700 text-xs px-3 py-2.5 rounded-lg"><span className="flex-shrink-0 mt-px">⚠️</span><span className="font-medium leading-relaxed">{msg}</span></div>;
}
...
function ModalWrapper({ children, onClose, preventClose, wide }: { children: React.ReactNode; onClose: () => void; preventClose?: boolean; wide?: boolean }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4"
      ...
      <div className={`w-full ${wide ? "sm:max-w-lg" : "sm:max-w-md"} bg-white rounded-t-2xl sm:rounded-xl shadow-xl overflow-hidden border border-gray-100`}
        style={{ animation: "modalUp 0.25s cubic-bezier(0.22,1,0.36,1)" }}
...
function ModalHead({ icon, title, sub, onClose, noClose }: { icon: string; title: string; sub?: string; onClose: () => void; noClose?: boolean }) {
  return (
    <div className="px-5 pt-4 pb-3.5 border-b border-gray-100 flex items-start gap-3">
      <div className="w-8 h-8 rounded-lg bg-gray-50 border border-gray-100 flex items-center justify-center text-base flex-shrink-0">{icon}</div>
      <div className="flex-1 min-w-0 pt-0.5"><h2 className="text-sm font-bold text-gray-900 leading-tight">{title}</h2>{sub && <p className="text-[10px] text-gray-400 mt-0.5 truncate">{sub}</p>}</div>
      <button onClick={onClose} disabled={noClose} className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all mt-0.5 ${noClose ? "text-gray-200 cursor-not-allowed" : "text-gray-400 hover:text-gray-700 hover:bg-gray-100"}`}>
...
function ModalFoot({ children }: { children: React.ReactNode }) {
  return <div className="px-5 py-3 border-t border-gray-100 bg-gray-50/50 flex gap-2">{children}</div>;
}
      ...
      <div className="px-5 py-4 space-y-3 max-h-[75vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[["Mulai", formatTime(o.actual_start ?? o.scheduled_start)], ["Selesai", formatTime(o.actual_end ?? o.scheduled_end)], ["Durasi", duration]].map(([k, v]) => (
            <div key={k} className="bg-gray-50 border border-gray-100 rounded-lg p-3 text-center">
        ...
        <div className="space-y-2">
          <div className="bg-gray-50 border border-gray-100 rounded-lg p-3">
            <p className="text-[9px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Alasan</p>
            <p className="text-xs text-gray-800">{o.reason || "—"}</p>
          </div>
          <div className="bg-gray-50 border border-gray-100 rounded-lg p-3">
            <p className="text-[9px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Rincian Pekerjaan</p>
            <p className="text-xs text-gray-700 leading-relaxed">{o.work_description || "—"}</p>
          </div>
        </div>
        {userCanViewPay && (
          <div className="bg-gray-900 rounded-lg p-3.5">
            <div className="flex items-end justify-between">
              <div><p className="text-[9px] text-gray-400 font-semibold uppercase tracking-wider mb-1">Total Bayaran</p><p className="text-lg font-bold text-white">{o.total_pay != null ? formatRupiah(o.total_pay) : "—"}</p></div>
              {o.rate_per_hour != null && <p className="text-[10px] text-gray-500">{formatRupiah(o.rate_per_hour)}/jam</p>}
            </div>
          </div>
        )}
        {o.proof_photo_url && (
          <div>
            <p className="text-[9px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Foto Bukti</p>
            <button onClick={() => { onClose(); setTimeout(onProofPhoto, 100); }} className="w-full overflow-hidden rounded-lg border border-gray-100 hover:opacity-90 transition-opacity">
              <img src={o.proof_photo_url} alt="Bukti" className="w-full h-40 object-cover" />
              <div className="px-3 py-1.5 bg-gray-50 text-center"><p className="text-[9px] text-gray-400">Tap untuk lihat penuh</p></div>
            </button>
          </div>
        )}
        {o.status === "REJECTED" && o.rejection_note && (
          <div className="bg-red-50 border border-red-100 rounded-lg p-3">
            <p className="text-[9px] font-semibold text-red-500 uppercase tracking-wider mb-1">Alasan Ditolak</p>
            <p className="text-xs text-red-700">{o.rejection_note}</p>
          </div>
        )}
        {o.approver && <div className="bg-gray-50 border border-gray-100 rounded-lg p-3"><p className="text-[9px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Disetujui oleh</p><p className="text-xs text-gray-800 font-semibold">{o.approver.name}</p></div>}
      </div>
      <ModalFoot>
        <div className="flex-1 flex gap-2 flex-wrap">
          ...
          {isAdminRole(currentUser?.role) && <>
            ...
            <button onClick={() => { onClose(); setTimeout(onDelete, 100); }} className="h-9 px-3 bg-white border border-red-100 text-red-500 rounded-lg text-xs font-semibold hover:bg-red-50 flex items-center">🗑️</button>
          </>}
          <button onClick={onClose} className="h-9 px-3 bg-gray-100 text-gray-600 rounded-lg text-xs font-semibold hover:bg-gray-200 transition-all">Tutup</button>
        </div>
      </ModalFoot>
    </ModalWrapper>
  );
    ...
    <div className="space-y-2.5">
      {error ? (
        <div className="rounded-lg bg-red-50 border border-red-200 p-4 text-center space-y-2">
          ...
          <label className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs font-semibold text-gray-700 cursor-pointer hover:bg-gray-50">
            🖼️ Pilih dari Galeri
        ...
        <div className="relative rounded-lg overflow-hidden bg-black aspect-video border border-gray-100">
          ...
          {!ready && <div className="absolute inset-0 flex items-center justify-center"><div className="w-6 h-6 border-2 border-white/30 border-t-white rounded-full animate-spin" /></div>}
          <button onClick={() => setFacing(p => p === "environment" ? "user" : "environment")} className="absolute top-2 right-2 w-7 h-7 rounded-lg bg-black/40 backdrop-blur-sm flex items-center justify-center text-white text-xs hover:bg-black/60 transition-all">🔄</button>
        </div>
      )}
      {processing && <div className="flex items-center gap-2 bg-gray-50 border border-gray-100 rounded-lg px-3 py-2"><div className="w-3.5 h-3.5 border-2 border-gray-300 border-t-gray-600 rounded-full animate-spin" /><p className="text-xs text-gray-500">Menambahkan watermark...</p></div>}
      <div className="flex gap-2">
        <button onClick={onCancel} className={secondaryBtn} style={{ flex: "0 0 auto", padding: "0 12px" }}>Batal</button>
        <button onClick={capture} disabled={!ready || processing || !!error} className={primaryBtn}>{processing ? <Spinner /> : <><span>📸</span><span>Ambil Foto</span></>}</button>
      </div>
    </div>
  );
}
...
function ProofPhotoModal({ overtime: o, onClose, canViewPay: showPay }: { overtime: OvertimeRequest; onClose: () => void; canViewPay?: boolean }) {
  ...
  return (
    <ModalWrapper onClose={onClose}>
      ...
      <div className="px-5 py-4 space-y-3 max-h-[75vh] overflow-y-auto">
        <img src={o.proof_photo_url} alt="Bukti" className="w-full h-56 object-cover rounded-lg border border-gray-100" />
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-gray-50 border border-gray-100 rounded-lg p-3"><p className="text-[9px] text-gray-400 font-semibold uppercase tracking-wider mb-1">Tanggal</p><p className="font-semibold text-gray-800 text-xs">{new Date(o.request_date).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" })}</p></div>
          <div className="bg-gray-50 border border-gray-100 rounded-lg p-3"><p className="text-[9px] text-gray-400 font-semibold uppercase tracking-wider mb-1">Waktu</p><p className="font-semibold text-gray-800 text-xs font-mono">{formatTime(o.actual_start ?? o.scheduled_start)} – {formatTime(o.actual_end ?? o.scheduled_end)}</p></div>
        </div>
        {showPay && o.total_pay != null && <div className="rounded-lg p-3 bg-gray-900 text-white"><p className="text-[9px] text-gray-400 font-semibold uppercase tracking-wider mb-1">Total Bayaran</p><p className="text-lg font-bold">{formatRupiah(o.total_pay)}</p></div>}
      </div>
    ...
    <ModalWrapper onClose={onClose}>
      ...
      <div className="px-5 py-4 space-y-3 max-h-[70vh] overflow-y-auto">
        ...
        <div className="bg-gray-50 border border-gray-100 rounded-lg p-3 space-y-1.5">
          <p className="text-[9px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Info Pengajuan</p>
          <div className="flex justify-between text-xs"><span className="text-gray-400">Tanggal</span><span className="font-semibold">{new Date(o.request_date).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}</span></div>
          <div className="flex justify-between text-xs"><span className="text-gray-400">Jam diminta</span><span className="font-mono font-semibold">{formatTime(o.requested_start)}</span></div>
          {o.reason && <div className="pt-2 border-t border-gray-200"><p className="text-[9px] text-gray-400 font-semibold uppercase tracking-wider mb-1">Alasan</p><p className="text-xs text-gray-700">{o.reason}</p></div>}
        </div>
        <div className="grid grid-cols-2 gap-2">
      ...
      {REASON_OPTIONS.map(opt => {
        ...
        return (
          <button key={opt.value} type="button" onClick={() => onChange(opt.value)}
            className={`flex items-start gap-2 px-3 py-2.5 rounded-lg border text-left transition-all ${sel ? "bg-violet-600 border-violet-600" : "bg-white border-gray-200 hover:border-violet-300 hover:bg-violet-50/50"}`}>
            <span className="text-sm flex-shrink-0 mt-px">{opt.icon}</span>
            <div><p className={`text-[11px] font-semibold leading-tight ${sel ? "text-white" : "text-gray-800"}`}>{opt.value}</p><p className={`text-[9px] mt-0.5 ${sel ? "text-violet-200" : "text-gray-400"}`}>{opt.desc}</p></div>
          </button>
        );
      })}
    </div>
  ...
  return (
    <ModalWrapper onClose={onClose}>
      ...
      <div className="px-5 py-4 space-y-3.5 max-h-[78vh] overflow-y-auto">
        ...
        <div><label className={lbl}>Rincian Pekerjaan *</label><textarea value={workDescription} onChange={e => setWorkDescription(e.target.value)} placeholder="Pekerjaan yang akan dikerjakan saat lembur..." rows={3} className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-xs bg-white focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 transition-all resize-none placeholder:text-gray-300" /></div>
      </div>
      <ModalFoot>
        <button onClick={onClose} className={secondaryBtn}>Batal</button>
      ...
      <div className="px-5 py-4 space-y-3 max-h-[65vh] overflow-y-auto">
        ...
        <div className="rounded-lg p-4 bg-gray-900 text-white">
          <div className="flex items-end justify-between mb-3">
            <div><p className="text-[9px] text-gray-400 font-semibold uppercase tracking-wider mb-1">Durasi Aktual</p><p className="text-3xl font-bold">{hours}<span className="text-sm font-normal text-gray-400 ml-1">jam</span></p></div>
            <div className="text-right text-xs"><p className="text-gray-500 text-[9px] mb-0.5">Mulai</p><p className="font-mono">{formatTime(o.actual_start)}</p><p className="text-gray-500 text-[9px] mt-1 mb-0.5">Selesai</p><p className="font-mono">{formatTime(o.actual_end)}</p></div>
          </div>
          <div className="border-t border-white/10 pt-3"><p className="text-[9px] text-gray-400 font-semibold uppercase tracking-wider mb-1">Total Bayaran</p><p className="text-lg font-bold text-emerald-400">{formatRupiah(Math.round(totalPay))}</p><p className="text-[9px] text-gray-500 mt-0.5">{hours} jam × {formatRupiah(Math.round(rate))}</p></div>
        </div>
        <div><label className={lbl}>Tarif Per Jam (Rp)</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] text-gray-400">Rp</span><input type="number" min={0} value={rate} onChange={e => setRate(parseFloat(e.target.value) || 0)} className="w-full h-9 border border-gray-200 rounded-lg pl-8 pr-3 text-xs font-mono bg-white focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 transition-all" /></div></div>
      </div>
      <ModalFoot>
        <button onClick={onClose} className={secondaryBtn}>Batal</button>
...
function CompleteModal({ overtime: o, onClose, onSaved, isAutoCompleted }: { overtime: OvertimeRequest; onClose: () => void; onSaved: () => void; isAutoCompleted?: boolean }) {
  ...
  return (
    <ModalWrapper onClose={handleClose} preventClose={mustUpload}>
      ...
      <div className="px-5 py-4 space-y-3 max-h-[75vh] overflow-y-auto">
        {isNeedProof && <div className="bg-orange-50 border border-orange-200 rounded-lg p-3"><p className="font-semibold text-orange-800 text-xs mb-0.5">📸 Foto Bukti Wajib</p><p className="text-[10px] text-orange-700">Upload foto bukti sebelum menutup halaman ini.</p></div>}
        {isAutoCompleted && !isNeedProof && <div className="bg-amber-50 border border-amber-200 rounded-lg p-3"><p className="font-semibold text-amber-800 text-xs mb-0.5">⚡ Waktu Lembur Habis</p><p className="text-[10px] text-amber-700">Lemburanmu selesai otomatis. Wajib upload foto bukti.</p></div>}
        {error && <ErrorBanner msg={error} />}
        {photoStep === "preview" && photoPreview && <div><img src={photoPreview} alt="Preview" className="w-full h-44 object-cover rounded-lg border border-gray-100 mb-2" /><button onClick={() => { setPhotoFile(null); setPhotoPreview(null); setPhotoStep("idle"); }} className="text-[10px] font-semibold text-gray-400 hover:text-gray-700">↺ Ambil Ulang</button></div>}
        ...
        {photoStep === "idle" && <button onClick={() => setPhotoStep("camera")} className={`w-full flex items-center justify-center gap-2.5 border-2 border-dashed rounded-lg p-6 text-center transition-all group ${mustUpload ? "border-orange-300 bg-orange-50/40 hover:border-orange-400" : "border-gray-200 hover:border-violet-300 hover:bg-violet-50/20"}`}><span className="text-2xl">📷</span><div className="text-left"><p className={`text-xs font-semibold ${mustUpload ? "text-orange-700" : "text-gray-700"}`}>Buka Kamera</p><p className={`text-[10px] mt-0.5 ${mustUpload ? "text-orange-500" : "text-gray-400"}`}>{mustUpload ? "Wajib ambil foto bukti" : "Ambil foto bukti lemburan"}</p></div></button>}
      </div>
      {photoStep !== "camera" && <ModalFoot>{!mustUpload && <button onClick={onClose} className={secondaryBtn}>Batal</button>}<button onClick={upload} disabled={uploading || (mustUpload && !photoFile)} className={primaryBtn}>{uploading ? <Spinner /> : photoFile ? "📸 Upload & Selesai" : mustUpload ? "📷 Ambil Foto Dulu" : "🏁 Selesai"}</button></ModalFoot>}
    </ModalWrapper>
  );
}
...
function ManualOvertimeModal({ onClose, onSaved, allUsers }: { onClose: () => void; onSaved: () => void; allUsers: User[] }) {
  ...
  return (
    <ModalWrapper onClose={onClose} wide>
      ...
      <div className="px-5 py-4 space-y-3 max-h-[75vh] overflow-y-auto">
        {error && <ErrorBanner msg={error} />}
        <div><label className={lbl}>Nama Karyawan *</label><select value={targetUserId} onChange={e => setTargetUserId(e.target.value)} className={inp + " cursor-pointer"}><option value="">— Pilih karyawan —</option>{allUsers.slice().sort((a, b) => a.name.localeCompare(b.name, "id-ID")).map(u => <option key={u.id} value={u.id}>{u.name} ({u.role.replace(/_/g, " ")})</option>)}</select></div>
        <div className="grid grid-cols-3 gap-2">
        ...
        {previewHours !== null && <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2"><span className="text-sm">⏱️</span><span className="text-xs text-gray-700">Durasi: <strong>{previewHours} jam</strong></span></div>}
        <div><label className={lbl}>Alasan Lembur *</label><ReasonGrid value={reasonType} onChange={v => { setReasonType(v); if (v !== "Lainnya") setReasonCustom(""); }} /></div>
        ...
        <div><label className={lbl}>Rincian Pekerjaan *</label><textarea value={workDescription} onChange={e => setWorkDescription(e.target.value)} placeholder="Pekerjaan yang dikerjakan saat lembur..." rows={2} className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-xs bg-white focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 transition-all resize-none placeholder:text-gray-300" /></div>
        <div>
    ...
    <ModalWrapper onClose={onClose} wide>
      ...
      <div className="px-5 py-4 space-y-3 max-h-[78vh] overflow-y-auto">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div><p className="text-[9px] text-gray-400 mb-1">Mulai</p><input type="time" value={startTime} onChange={e => setStartTime(e.target.value)} className={inp} /></div>
            <div><p className="text-[9px] text-gray-400 mb-1">Selesai</p><input type="time" value={endTime} onChange={e => setEndTime(e.target.value)} className={inp} /></div>
          </div>
          {prevDur !== null && <div className="mt-2 inline-flex items-center gap-1.5 bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5"><span className="text-xs">⏱️</span><span className="text-[10px] text-gray-700">{prevDur} jam</span></div>}
        </div>
        <div><label className={lbl}>Alasan Lembur *</label><ReasonGrid value={reasonType} onChange={v => { setReasonType(v); if (v !== "Lainnya") setReasonCustom(""); }} /></div>
        {isLainnya && <div><label className={lbl}>Jelaskan Alasan *</label><input type="text" value={reasonCustom} onChange={e => setReasonCustom(e.target.value)} placeholder="Tuliskan alasan spesifik..." className={inp} autoFocus /></div>}
        <div><label className={lbl}>Rincian Pekerjaan</label><textarea value={workDesc} onChange={e => setWorkDesc(e.target.value)} rows={2} placeholder="Pekerjaan yang dikerjakan saat lembur..." className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-xs bg-white focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 transition-all resize-none placeholder:text-gray-300" /></div>
        <div>
          <label className={lbl}>Foto Bukti <span className="normal-case font-normal text-gray-400">(opsional)</span></label>
...
function DeleteConfirmModal({ overtime: o, onClose, onDeleted, canViewPay: showPay }: { overtime: OvertimeRequest; onClose: () => void; onDeleted: () => void; canViewPay?: boolean }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/attendance/overtime?id=${o.id}`, { method: "DELETE" }); const d = await res.json();
    ...
    <ModalWrapper onClose={onClose}>
      ...
      <div className="px-5 py-4 space-y-3">
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-3.5 space-y-2">
          {[["Karyawan", o.users?.name ?? "—"], ["Tanggal", new Date(o.request_date).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })], ["Waktu", `${formatTime(o.scheduled_start)} – ${formatTime(o.scheduled_end)}`]].map(([k, v]) => (
            <div key={k} className="flex justify-between text-xs"><span className="text-gray-400">{k}</span><span className="font-semibold text-gray-800">{v}</span></div>
          ))}
          <div className="flex justify-between items-center text-xs pt-1 border-t border-gray-200"><span className="text-gray-400">Status</span><StatusBadge status={o.status} /></div>
          {showPay && o.total_pay != null && <div className="flex justify-between text-xs"><span className="text-gray-400">Total Bayaran</span><span className="font-semibold text-gray-800">{formatRupiah(o.total_pay)}</span></div>}
        </div>
        <p className="text-[10px] text-gray-400 text-center">Tindakan ini tidak dapat dibatalkan.</p>
      </div>
      <ModalFoot>
        <button onClick={onClose} className={secondaryBtn}>Batal</button>
        <button onClick={handleDelete} disabled={deleting} className="flex-1 h-9 bg-red-600 hover:bg-red-700 text-white rounded-lg text-xs font-semibold transition-all flex items-center justify-center gap-1.5 disabled:opacity-40">{deleting ? <Spinner /> : "🗑️ Hapus Permanen"}</button>
      </ModalFoot>
    </ModalWrapper>
  );
}
...
// Muncul ketika nama karyawan diklik — menggantikan daftar karyawan
function EmployeeDetailView({ userId, name, role, overtimes, userCanViewPay, currentUser, onBack, onDetailOpen }: {
  ...
  const counts = { total: overtimes.length, pending: overtimes.filter(o => o.status === "PENDING").length, ongoing: overtimes.filter(o => o.status === "ONGOING").length, selesai: overtimes.filter(o => o.status === "COMPLETED").length };

  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      {/* Header */}
      <div className="px-5 py-4 border-b border-gray-100 flex items-center gap-3">
        <button onClick={onBack} className="w-8 h-8 rounded-lg border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-gray-50 hover:text-gray-800 transition-all flex-shrink-0">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
        </button>
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold flex-shrink-0 ${bg}`}>{initials(name)}</div>
      ...
      <div className="px-5 py-3 border-b border-gray-100 bg-gray-50/50">
        <div className="grid grid-cols-4 divide-x divide-gray-200">
          {[["Total", counts.total, "text-gray-800"], ["Pending", counts.pending, "text-amber-600"], ["Berjalan", counts.ongoing, "text-emerald-600"], ["Selesai", counts.selesai, "text-blue-600"]].map(([l, v, c]) => (
            <div key={String(l)} className="text-center px-2">
          ...
          {["Semua", ...statuses].map(s => {
            ...
            return (
              <button key={s} onClick={() => setFilterStatus(s)}
                className={`flex-shrink-0 h-7 px-2.5 rounded-md text-[10px] font-semibold transition-all border ${active ? "bg-gray-900 text-white border-gray-900" : "bg-white text-gray-500 border-gray-200 hover:border-gray-300 hover:text-gray-700"}`}>
      ...
      {/* List */}
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 gap-2"><span className="text-2xl">📭</span><p className="text-xs text-gray-400 font-medium">Tidak ada data</p></div>
      ) : (
          ...
          {filtered.map(o => {
            ...
            return (
              <button key={o.id} onClick={() => onDetailOpen(o)}
                className={`w-full px-5 py-3.5 flex items-center gap-4 text-left hover:bg-gray-50 transition-colors group ${isMyUrgent ? "bg-orange-50/20" : ""}`}>
                <div className="w-9 flex-shrink-0 text-center">
                  <p className="text-base font-bold text-gray-800 leading-none">{dateObj.getDate()}</p>
                  <p className="text-[9px] text-gray-400 font-medium uppercase mt-0.5">{MONTH_NAMES[dateObj.getMonth()].substring(0, 3)}</p>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 text-[10px] text-gray-500">
                    ...
                    {duration !== "—" && <><span className="text-gray-300">·</span><span>{duration}</span></>}
                  </div>
                  {o.reason && <p className="text-[10px] text-gray-400 truncate mt-0.5">{o.reason}</p>}
                </div>
                <div className="flex-shrink-0 flex items-center gap-2">
                  {userCanViewPay && o.total_pay != null && <p className="text-xs font-semibold text-gray-700 font-mono">{formatRupiah(o.total_pay)}</p>}
...
// ─── EMPLOYEE LIST PANEL ───────────────────────────────────────────────────
// Panel utama: semua karyawan langsung muncul sebagai baris clickable
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="px-5 py-3.5 border-b border-gray-100 space-y-2.5">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] text-gray-400 bg-gray-100 px-2.5 py-1 rounded-full tabular-nums">{groupedByUser.length} karyawan</span>
        </div>
        <div className="flex gap-2">
          <div className="relative flex-1">
            <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><circle cx="11" cy="11" r="8" /><path d="m21 21-4.35-4.35" /></svg>
            <input type="text" placeholder="Cari nama..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full h-8 pl-8 pr-3 border border-gray-200 rounded-lg text-xs bg-white focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 transition-all placeholder:text-gray-300" />
          </div>
          <div className="flex gap-1 overflow-x-auto scrollbar-hide">
            {["Semua", ...statuses].map(s => {
              const c = s !== "Semua" ? STATUS_CONFIG[s] : null; const active = filterStatus === s;
              return <button key={s} onClick={() => setFilterStatus(s)} className={`flex-shrink-0 h-8 px-2.5 rounded-lg text-[10px] font-semibold transition-all border ${active ? "bg-gray-900 text-white border-gray-900" : "bg-white text-gray-500 border-gray-200 hover:border-gray-300 hover:text-gray-700"}`}>{c ? `${c.icon} ${c.label}` : "Semua"}</button>;
            })}
          </div>
        </div>
      </div>
        ...
        <div className="divide-y divide-gray-50">
          {Array(5).fill(0).map((_, i) => (
            <div key={i} className="px-5 py-3.5 flex items-center gap-3">
              ...
              <div className="flex-1 space-y-1.5"><div className="h-3 bg-gray-100 rounded animate-pulse w-32" /><div className="h-2.5 bg-gray-100 rounded animate-pulse w-20" /></div>
            </div>
          ))}
        </div>
      ) : groupedByUser.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 gap-2"><span className="text-2xl">📭</span><p className="text-xs font-medium text-gray-400">Tidak ada data lemburan</p><p className="text-[10px] text-gray-300">Coba ubah filter atau hapus pencarian</p></div>
      ) : (
        <div className="divide-y divide-gray-50">
          {groupedByUser.map(({ user, items }) => {
            ...
            return (
              <button key={user.id} onClick={() => onSelectUser(user.id)} className="w-full px-5 py-3.5 flex items-center gap-3 hover:bg-gray-50 transition-colors text-left group">
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center text-xs font-bold flex-shrink-0 ${bg}`}>{initials(user.name)}</div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-gray-400 mt-0.5">{user.role.replace(/_/g, " ")}</p>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <div className="hidden sm:flex items-center gap-1">
                    {Object.entries(statusCounts).slice(0, 2).map(([s, c]) => { const cfg = STATUS_CONFIG[s]; if (!cfg) return null; return <span key={s} className={`text-[8px] font-semibold px-1.5 py-0.5 rounded border ${cfg.bg} ${cfg.text} ${cfg.border}`}>{c} {cfg.label}</span>; })}
                  </div>
                  {userCanViewPay && totalPay > 0 && <p className="hidden sm:block text-[10px] font-semibold text-gray-600 font-mono">{formatRupiah(totalPay)}</p>}
                  <div className="w-5 h-5 rounded-full bg-gray-100 flex items-center justify-center text-[9px] font-bold text-gray-600">{items.length}</div>
                  <svg className="w-4 h-4 text-gray-300 group-hover:text-gray-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
                </div>
              </button>
            );
...
export default function OvertimePage() {
  ...
  // null = tampil daftar, string = tampil detail karyawan itu
  ...
  // filtered: data lembur yang lolos filter status (tanpa name filter)
  ...
  const groupedByUser = useMemo(() => {
    ...
    // Seed dari allUsers — filter nama di sini agar user tanpa lembur ikut terfilter
    allUsers.forEach(u => {
    ...
    // Masukkan data lembur ke user yang ada di map
    filtered.forEach(o => {
      if (!o.users) return;
      const uid = o.user_id;
      if (map.has(uid)) {
        map.get(uid)!.items.push(o);
      } else if (!q || o.users.name.toLowerCase().includes(q)) {
        // User tidak ada di allUsers tapi punya lembur dan lolos filter nama
        map.set(uid, { user: o.users, items: [o] });
      }
    });

    // Jika filter status aktif, sembunyikan user yang tidak punya item lolos filter
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <div className="flex items-center gap-2.5 mb-0.5">
                <div className="w-1 h-5 rounded-full bg-violet-600 flex-shrink-0" />
                <h1 className="text-lg sm:text-xl font-bold text-gray-900">Lembur Karyawan</h1>
              </div>
              <p className="text-xs text-gray-400 pl-4">{loading ? "Memuat data..." : `${overtimes.length} lemburan · ${groupedByUser.length} karyawan`}</p>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              {isAdminRole(currentUser?.role) && (
                <button onClick={() => setShowManualModal(true)} className="h-8 px-3 bg-white border border-gray-200 hover:border-gray-300 text-gray-700 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all whitespace-nowrap">
                  <span>✏️</span><span className="hidden sm:inline">Input Manual</span>
                </button>
              )}
              <button onClick={() => setShowRequestModal(true)} className="h-8 px-3 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all whitespace-nowrap">
          ...
          <div className="grid grid-cols-4 gap-3">
            {statCards.map(c => (
              <div key={c.label} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-gray-50 border border-gray-100 flex items-center justify-center text-sm flex-shrink-0">{c.icon}</div>
                <div className="min-w-0">
                  <p className="text-[9px] text-gray-400 font-medium uppercase tracking-widest leading-none mb-1">{c.label}</p>
                  <p className={`text-xl font-bold leading-none ${c.color}`}>{loading ? <span className="inline-block w-6 h-4 bg-gray-100 rounded animate-pulse" /> : c.value}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100">
              <div className="flex items-center gap-2.5">
                <div className="w-7 h-7 rounded-lg bg-gray-50 border border-gray-100 flex items-center justify-center text-sm flex-shrink-0">📅</div>
                <div>
                  <p className="text-[9px] font-semibold text-gray-400 uppercase tracking-widest leading-none mb-0.5">Kalender Lembur</p>
                  <p className="font-bold text-gray-900 text-sm">{MONTH_NAMES[calendarMonth.month]} {calendarMonth.year}</p>
                </div>
              </div>
              <div className="flex items-center gap-1">
                <button onClick={() => setCalendarMonth(m => ({ month: m.month === 0 ? 11 : m.month - 1, year: m.month === 0 ? m.year - 1 : m.year }))} className="w-7 h-7 rounded-lg hover:bg-gray-100 flex items-center justify-center text-gray-500 hover:text-gray-800 transition-all text-sm font-bold">‹</button>
                <button onClick={() => setCalendarMonth({ year: new Date().getFullYear(), month: new Date().getMonth() })} className="h-7 px-2.5 rounded-lg hover:bg-gray-100 text-[10px] font-semibold text-gray-500 hover:text-gray-800 transition-all">Hari ini</button>
                <button onClick={() => setCalendarMonth(m => ({ month: m.month === 11 ? 0 : m.month + 1, year: m.month === 11 ? m.year + 1 : m.year }))} className="w-7 h-7 rounded-lg hover:bg-gray-100 flex items-center justify-center text-gray-500 hover:text-gray-800 transition-all text-sm font-bold">›</button>
              </div>
            </div>
            <div className="p-4 overflow-x-auto">
              <div className="grid grid-cols-7 gap-1 mb-1.5 min-w-[280px]">
                {DAY_NAMES.map((d, i) => <div key={d} className={`text-center text-[9px] font-semibold py-1 uppercase tracking-wider ${i === 0 ? "text-red-400" : "text-gray-400"}`}>{d}</div>)}
              </div>
                ...
                {calDays.map((day, idx) => {
                  ...
                  return (
                    <button key={day} onClick={() => handleDateSelect(dk)}
                      className={`relative flex flex-col items-center justify-start pt-1.5 pb-1 rounded-xl min-h-[50px] sm:min-h-[60px] transition-all border ${isSel ? "bg-gray-900 border-gray-900" : total > 0 ? "bg-violet-50 border-violet-100 hover:bg-violet-100" : "bg-transparent border-gray-100 hover:bg-gray-50"}`}>
                      <span className={`font-bold text-sm leading-none mb-1 ${isSel ? "text-white" : isToday ? "text-violet-600" : isSun ? "text-red-400" : "text-gray-700"}`}>{day}</span>
                      {isToday && !isSel && <span className="absolute top-1 right-1 w-1.5 h-1.5 rounded-full bg-violet-500" />}
                      {total > 0 && <span className={`text-[9px] font-bold rounded-full px-1.5 py-0.5 leading-none ${isSel ? "bg-white/20 text-white" : "bg-violet-500 text-white"}`}>{total}</span>}
                    </button>
                  );
                })}
              </div>
            ...
            {selectedDate && (
              <div className="px-4 pb-3">
                <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                  <p className="text-xs text-gray-600 flex-1"><span className="font-semibold">{new Date(selectedDate + "T12:00:00").toLocaleDateString("id-ID", { weekday: "long", day: "numeric", month: "long" })}</span><span className="text-gray-400 ml-1">· {(byDate[selectedDate] || []).length} lemburan</span></p>
                  <button onClick={() => setSelectedDate(null)} className="text-[10px] text-gray-400 hover:text-gray-700 font-semibold">✕ Hapus</button>
                </div>
              </div>
            )}
          </div>

          {/* Employee List / Detail — posisi tetap di bawah kalender */}
