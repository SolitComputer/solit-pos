...
const PAY_VIEW_ROLES = [
  "KEPALA_SALES", "KEPALA_MARKETING", "KEPALA_TEKNISI", "KEPALA_PENYEDIA_BARANG",
  "ADMIN", "PROGRAMMER", "ASISTEN_CEO", "KEPALA_ONPOINT", "KEPALA_SOTECH",
] as const;
...
function CameraCapture({ onCapture, onCancel }: CameraCaptureProps) {
  ...
  const startCamera = useCallback(async (facing: "environment" | "user") => {
    ...
    try {
      ...
      const msg =
        err.name === "NotAllowedError" ? "Izin kamera ditolak. Aktifkan izin kamera di browser."
  ...
  return (
    <div className="space-y-3">
      {error ? (
        ...
        <div className="relative rounded-2xl overflow-hidden bg-black aspect-video border border-gray-100 shadow-inner">
          ...
          <button onClick={() => setFacingMode((p) => p === "environment" ? "user" : "environment")}
            className="absolute top-3 right-3 w-9 h-9 rounded-xl bg-black/40 backdrop-blur-sm flex items-center justify-center text-white text-base hover:bg-black/60 transition-all active:scale-95" title="Ganti kamera">
            🔄
          </button>
          {isReady && (
      ...
      <div className="flex gap-2">
        <button onClick={onCancel} className={secondaryBtn} style={{ flex: "0 0 auto", width: "auto", padding: "0 16px" }}>Batal</button>
        <button onClick={capture} disabled={!isReady || isProcessing || !!error} className={primaryBtn}>
...
function StatusBadge({ status }: { status: OvertimeRequest["status"] }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1 rounded-full border whitespace-nowrap ${cfg.bg} ${cfg.text} ${cfg.border}`}>
      ...
      <div className="px-6 py-5 space-y-4 max-h-[75vh] overflow-y-auto">
        ...
        <div className="bg-violet-50 border border-violet-100 rounded-2xl p-4 space-y-2.5">
          ...
          <div className="space-y-2 text-sm text-gray-700">
            ...
            {overtime.reason && (<div className="pt-1.5 border-t border-violet-100"><p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-1">Alasan</p><p className="text-xs text-gray-700 font-semibold">{overtime.reason}</p></div>)}
            {overtime.work_description && (<div className="pt-1.5 border-t border-violet-100"><p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-1">Rincian Pekerjaan</p><p className="text-xs text-gray-700 leading-relaxed">{overtime.work_description}</p></div>)}
          </div>
        </div>
        <div className="flex items-start gap-3 bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-3">
          <span className="text-base flex-shrink-0">⚡</span>
          <p className="text-xs text-emerald-700 font-medium leading-relaxed">Setelah disetujui, lembur akan <strong>langsung berjalan otomatis</strong> — karyawan tidak perlu menekan tombol Mulai.</p>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div><label className={labelCls}>Jam Mulai <span className="text-red-400 normal-case tracking-normal">*</span></label><input type="time" value={scheduledStart} onChange={(e) => setScheduledStart(e.target.value)} className={inputCls} /></div>
          <div><label className={labelCls}>Jam Selesai <span className="text-red-400 normal-case tracking-normal">*</span></label><input type="time" value={scheduledEnd} onChange={(e) => setScheduledEnd(e.target.value)} className={inputCls} /></div>
        </div>
      </div>
      <ModalFooter>
        <button onClick={onClose} className={secondaryBtn}>Batal</button>
        <button onClick={approve} disabled={approving || !scheduledStart || !scheduledEnd} className={primaryBtn}>{approving ? <Spinner /> : "✅ Setujui & Mulai"}</button>
      </ModalFooter>
    </ModalWrapper>
  );
}
  ...
  return (
    <ModalWrapper onClose={onClose}>
      ...
      <div className="px-6 py-5 space-y-5 max-h-[78vh] overflow-y-auto">
        {error && <ErrorBanner msg={error} />}
        <div><label className={labelCls}>Tanggal Lembur <span className="text-red-400 normal-case tracking-normal">*</span></label><input type="date" min={today} value={requestDate} onChange={(e) => setRequestDate(e.target.value)} className={inputCls} /></div>
        <div><label className={labelCls}>Jam Mulai <span className="text-red-400 normal-case tracking-normal">*</span></label><input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className={inputCls} /></div>
        <div>
          <label className={labelCls}>Alasan Lembur <span className="text-red-400 normal-case tracking-normal">*</span></label>
          <div className="space-y-2">
            {REASON_OPTIONS.map((opt) => {
              const isSel = reasonType === opt.value;
              return (
                <button key={opt.value} type="button" onClick={() => { setReasonType(opt.value); if (opt.value !== "Lainnya") setReasonCustom(""); }}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border text-left transition-all active:scale-[0.99] ${isSel ? "bg-gray-900 border-gray-900 shadow-sm" : "bg-white border-gray-200 hover:border-gray-300 hover:bg-gray-50"}`}>
                  <span className="text-lg flex-shrink-0">{opt.icon}</span>
                  <div className="min-w-0">
                    <p className={`text-sm font-bold leading-tight ${isSel ? "text-white" : "text-gray-800"}`}>{opt.value}</p>
                    <p className={`text-[10px] mt-0.5 leading-tight ${isSel ? "text-gray-300" : "text-gray-400"}`}>{opt.desc}</p>
                  </div>
                  {isSel && <span className="ml-auto flex-shrink-0 w-5 h-5 rounded-full bg-white/20 flex items-center justify-center text-white text-xs font-black">✓</span>}
                </button>
              );
            })}
          </div>
        </div>
        {isLainnya && (<div><label className={labelCls}>Jelaskan Alasan <span className="text-red-400 normal-case tracking-normal">*</span></label><input type="text" value={reasonCustom} onChange={(e) => setReasonCustom(e.target.value)} placeholder="Tuliskan alasan spesifik kamu..." className={inputCls} autoFocus /></div>)}
        <div>
          <label className={labelCls}>Rincian Pekerjaan <span className="text-red-400 normal-case tracking-normal">*</span></label>
          <textarea value={workDescription} onChange={(e) => setWorkDescription(e.target.value)} placeholder="Jelaskan pekerjaan yang akan dikerjakan saat lembur..." rows={4} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 transition-all resize-none placeholder:text-gray-300" />
          <p className="text-[10px] text-gray-400 mt-1.5 font-medium">Contoh: Menyelesaikan desain konten Shopee promo weekend, input stok laptop baru, dll.</p>
        </div>
      </div>
      <ModalFooter>
        <button onClick={onClose} className={secondaryBtn}>Batal</button>
        <button onClick={submit} disabled={submitting || !canSubmit} className={primaryBtn}>{submitting ? <><Spinner /><span>Mengirim...</span></> : "📝 Ajukan"}</button>
      </ModalFooter>
    </ModalWrapper>
  );
}
      ...
      <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
        ...
        <div className="rounded-2xl bg-gradient-to-br from-gray-900 to-gray-800 p-5 text-white">
          <div className="flex items-center justify-between mb-4">
            <div><p className="text-xs text-gray-400 font-medium">Durasi aktual</p><p className="text-3xl font-black mt-0.5">{hours} <span className="text-lg font-bold text-gray-300">jam</span></p></div>
            <div className="text-right"><p className="text-xs text-gray-400">Mulai</p><p className="text-sm font-bold font-mono">{formatTime(overtime.actual_start)}</p><p className="text-xs text-gray-400 mt-1">Selesai</p><p className="text-sm font-bold font-mono">{formatTime(overtime.actual_end)}</p></div>
          </div>
        ...
        <div>
          ...
          <div className="relative"><span className="absolute left-4 top-1/2 -translate-y-1/2 text-xs text-gray-400 font-bold">Rp</span><input type="number" min={0} value={rate} onChange={(e) => setRate(parseFloat(e.target.value) || 0)} className="w-full h-11 border border-gray-200 rounded-xl pl-11 pr-4 text-sm font-mono bg-white focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 transition-all" /></div>
        </div>
      </div>
      <ModalFooter>
        <button onClick={onClose} className={secondaryBtn}>Batal</button>
...
function CompleteModal({ overtime, onClose, onSaved, isAutoCompleted }: { overtime: OvertimeRequest; onClose: () => void; onSaved: () => void; isAutoCompleted?: boolean }) {
  ...
  const upload = async () => {
    if (mustUpload && !photoFile) { setError("⚠️ Foto bukti wajib diupload sebelum melanjutkan."); return; }
  ...
  return (
    <ModalWrapper onClose={handleClose} preventClose={mustUpload}>
      <ModalHeader icon={isNeedProof ? "📸" : "🏁"} title={isNeedProof ? "Upload Bukti Lemburan" : "Selesaikan Lemburan"} subtitle={overtime.users?.name} onClose={handleClose} disableClose={mustUpload} />
      <div className="px-6 py-5 space-y-4 max-h-[78vh] overflow-y-auto">
        {isNeedProof && (<div className="bg-orange-50 border border-orange-200 rounded-2xl p-4"><p className="font-black text-orange-800 text-sm mb-1">📸 Upload Foto Bukti Wajib!</p><p className="text-xs text-orange-700 leading-relaxed">Lemburanmu sudah selesai tapi belum ada foto bukti. Kamu <strong>wajib</strong> upload foto sebelum menutup halaman ini.</p></div>)}
        {isAutoCompleted && !isNeedProof && (<div className="bg-amber-50 border border-amber-200 rounded-2xl p-4"><p className="font-black text-amber-800 text-sm mb-1">⚡ Waktu Lembur Habis!</p><p className="text-xs text-amber-700 leading-relaxed">Lemburanmu selesai otomatis. Kamu <strong>wajib</strong> upload foto bukti sebelum menutup halaman ini.</p></div>)}
        {error && <ErrorBanner msg={error} />}
        {photoStep === "preview" && photoPreview && (
          <div><label className={labelCls}>Foto Bukti</label><img src={photoPreview} alt="Preview" className="w-full h-52 object-cover rounded-2xl border border-gray-100 shadow-md mb-3" /><button onClick={() => { setPhotoFile(null); setPhotoPreview(null); setPhotoStep("idle"); }} className="text-xs font-bold text-gray-400 hover:text-gray-700 transition-colors">↺ Ambil Ulang</button></div>
        )}
        {photoStep === "camera" && (<div><label className={labelCls}>Kamera</label><CameraCapture onCapture={(file, url) => { setPhotoFile(file); setPhotoPreview(url); setPhotoStep("preview"); setError(""); }} onCancel={() => setPhotoStep("idle")} /></div>)}
        {photoStep === "idle" && (
          <div>
            <label className={labelCls}>Foto Bukti{" "}{mustUpload ? <span className="text-red-400 normal-case tracking-normal font-bold">*</span> : <span className="normal-case tracking-normal text-gray-400 font-normal">(opsional)</span>}</label>
            <button onClick={() => setPhotoStep("camera")} className={`w-full flex items-center justify-center gap-3 border-2 border-dashed rounded-2xl p-8 text-center transition-all active:scale-[0.99] group ${mustUpload ? "border-orange-300 bg-orange-50/40 hover:border-orange-400 hover:bg-orange-50" : "border-gray-200 hover:border-violet-300 hover:bg-violet-50/30"}`}>
              <span className="text-3xl group-hover:scale-110 transition-transform">📷</span>
              <div className="text-left">
                <p className={`text-sm font-bold ${mustUpload ? "text-orange-700" : "text-gray-700"}`}>Buka Kamera</p>
                <p className={`text-xs mt-0.5 ${mustUpload ? "text-orange-500" : "text-gray-400"}`}>{mustUpload ? "Wajib ambil foto bukti lemburan" : "Ambil foto bukti lemburan langsung"}</p>
              </div>
            </button>
          </div>
        )}
...
function ManualOvertimeModal({ onClose, onSaved, allUsers }: { onClose: () => void; onSaved: () => void; allUsers: User[] }) {
  ...
  const calcPreview = () => {
    ...
    const s = new Date(`1970-01-01T${startTime}:00`).getTime();
    const e = new Date(`1970-01-01T${endTime}:00`).getTime();
    if (e <= s) return null;
    return Math.floor((e - s) / (60 * 60 * 1000));
  };
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (photoFile) {
        ...
        const uploadRes = await fetch("/api/attendance/overtime/upload", { method: "POST", body: formData });
        if (!uploadRes.ok) { const err = await uploadRes.json(); throw new Error(err.message || "Upload foto gagal"); }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(8px)" }}>
      <div className="w-full sm:max-w-lg bg-white rounded-t-[2rem] sm:rounded-2xl shadow-2xl overflow-hidden" style={{ animation: "modalUp 0.32s cubic-bezier(0.22,1,0.36,1)" }} onClick={(e) => e.stopPropagation()}>
        <ModalHeader icon="✏️" title="Input Lembur Manual" subtitle="Admin · Asisten CEO · Programmer" onClose={onClose} />
        <div className="px-6 py-5 space-y-4 max-h-[75vh] overflow-y-auto">
          {error && <ErrorBanner msg={error} />}
          <div><label className={labelCls}>Nama Karyawan <span className="text-red-400 normal-case tracking-normal">*</span></label><select value={targetUserId} onChange={(e) => setTargetUserId(e.target.value)} className={inputCls + " cursor-pointer"}><option value="">— Pilih karyawan —</option>{allUsers.slice().sort((a, b) => a.name.localeCompare(b.name, "id-ID")).map((u) => (<option key={u.id} value={u.id}>{u.name} ({u.role.replace(/_/g, " ")})</option>))}</select></div>
          <div><label className={labelCls}>Tanggal Lembur <span className="text-red-400 normal-case tracking-normal">*</span></label><input type="date" value={requestDate} onChange={(e) => setRequestDate(e.target.value)} className={inputCls} /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className={labelCls}>Jam Mulai <span className="text-red-400 normal-case tracking-normal">*</span></label><input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className={inputCls} /></div>
            <div><label className={labelCls}>Jam Selesai <span className="text-red-400 normal-case tracking-normal">*</span></label><input type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)} className={inputCls} /></div>
          </div>
          {previewHours !== null && (<div className="flex items-center gap-3 bg-gray-50 border border-gray-100 rounded-xl px-4 py-3"><span className="text-lg">⏱️</span><p className="text-sm text-gray-600">Durasi: <span className="font-black text-gray-900">{previewHours} jam</span>{previewHours === 0 && <span className="text-amber-500 ml-2 text-xs font-bold">(kurang dari 1 jam)</span>}</p></div>)}
          <div>
            <label className={labelCls}>Alasan Lembur <span className="text-red-400 normal-case tracking-normal">*</span></label>
            <div className="space-y-2">
              {REASON_OPTIONS.map((opt) => {
                const isSel = reasonType === opt.value;
                return (<button key={opt.value} type="button" onClick={() => { setReasonType(opt.value); if (opt.value !== "Lainnya") setReasonCustom(""); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border text-left transition-all active:scale-[0.99] ${isSel ? "bg-gray-900 border-gray-900 shadow-sm" : "bg-white border-gray-200 hover:border-gray-300 hover:bg-gray-50"}`}><span className="text-lg flex-shrink-0">{opt.icon}</span><div className="min-w-0"><p className={`text-sm font-bold leading-tight ${isSel ? "text-white" : "text-gray-800"}`}>{opt.value}</p><p className={`text-[10px] mt-0.5 leading-tight ${isSel ? "text-gray-300" : "text-gray-400"}`}>{opt.desc}</p></div>{isSel && <span className="ml-auto flex-shrink-0 w-5 h-5 rounded-full bg-white/20 flex items-center justify-center text-white text-xs font-black">✓</span>}</button>);
              })}
            </div>
          </div>
          {reasonType === "Lainnya" && (<div><label className={labelCls}>Jelaskan Alasan <span className="text-red-400 normal-case tracking-normal">*</span></label><input type="text" value={reasonCustom} onChange={(e) => setReasonCustom(e.target.value)} placeholder="Tuliskan alasan spesifik..." className={inputCls} autoFocus /></div>)}
          <div><label className={labelCls}>Rincian Pekerjaan <span className="text-red-400 normal-case tracking-normal">*</span></label><textarea value={workDescription} onChange={(e) => setWorkDescription(e.target.value)} placeholder="Jelaskan pekerjaan yang dikerjakan saat lembur..." rows={3} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 transition-all resize-none placeholder:text-gray-300" /><p className="text-[10px] text-gray-400 mt-1.5 font-medium">Contoh: Menyelesaikan desain konten Shopee promo weekend, input stok laptop baru, dll.</p></div>
          <div>
            ...
            {photoStep === "preview" && photoPreview ? (<div><img src={photoPreview} alt="Preview" className="w-full h-44 object-cover rounded-2xl border border-gray-100 shadow-md mb-3" /><button onClick={() => { setPhotoFile(null); setPhotoPreview(null); setPhotoStep("idle"); }} className="text-xs font-bold text-gray-400 hover:text-gray-700 transition-colors">↺ Ambil Ulang</button></div>)
            ...
            : (<button onClick={() => setPhotoStep("camera")} className="w-full flex items-center justify-center gap-3 border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center hover:border-violet-300 hover:bg-violet-50/30 transition-all active:scale-[0.99] group"><span className="text-2xl group-hover:scale-110 transition-transform">📷</span><div className="text-left"><p className="text-sm font-bold text-gray-700">Buka Kamera</p><p className="text-xs text-gray-400 mt-0.5">Ambil foto bukti langsung</p></div></button>)}
          </div>
        </div>
        {photoStep !== "camera" && (
          <ModalFooter>
            <button onClick={onClose} className={secondaryBtn}>Batal</button>
            <button onClick={submit} disabled={submitting || !targetUserId || !requestDate || !startTime || !endTime || !reasonType || (reasonType === "Lainnya" && !reasonCustom.trim()) || !workDescription.trim()} className={primaryBtn}>{submitting ? <><Spinner /><span>Menyimpan...</span></> : "✅ Simpan Lembur"}</button>
          </ModalFooter>
        )}
      </div>
    </div>
  ...
  const save = async () => {
    ...
    try {
      ...
      if (photoFile) {
        ...
        const uploadRes = await fetch("/api/attendance/overtime/upload", { method: "POST", body: formData });
        if (!uploadRes.ok) { const err = await uploadRes.json(); throw new Error(err.message || "Upload foto gagal"); }
        const { url } = await uploadRes.json(); proofUrl = url;
      }
      const isoStart = `${requestDate}T${fmt(startTime)}+07:00`;
      const isoEnd = `${requestDate}T${fmt(endTime)}+07:00`;
      const payload: Record<string, any> = { id: overtime.id, action: "UPDATE", request_date: requestDate, scheduled_start: isoStart, scheduled_end: isoEnd, actual_start: isoStart, actual_end: isoEnd, reason: isLainnya ? reasonCustom.trim() : reasonType, work_description: workDesc.trim(), status };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(8px)" }}>
      <div className="w-full sm:max-w-lg bg-white rounded-t-[2rem] sm:rounded-2xl shadow-2xl overflow-hidden" style={{ animation: "modalUp 0.32s cubic-bezier(0.22,1,0.36,1)" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-5 space-y-4 max-h-[78vh] overflow-y-auto">
          {error && <ErrorBanner msg={error} />}
          <div><label className={labelCls}>Tanggal <span className="text-red-400 normal-case">*</span></label><input type="date" value={requestDate} onChange={(e) => setRequestDate(e.target.value)} className={inputCls} /></div>
          <div><label className={labelCls}>Jam Lembur <span className="text-red-400 normal-case">*</span></label><div className="grid grid-cols-2 gap-3"><div><p className="text-[10px] text-gray-400 font-semibold mb-1.5">Mulai</p><input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className={inputCls} /></div><div><p className="text-[10px] text-gray-400 font-semibold mb-1.5">Selesai</p><input type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)} className={inputCls} /></div></div></div>
          {previewDuration !== null && (<div className="flex items-center gap-2 bg-gray-50 border border-gray-100 rounded-xl px-4 py-3"><span>⏱️</span><span className="text-sm text-gray-600">Durasi: <span className="font-black text-gray-900">{previewDuration} jam</span></span></div>)}
          <div>
            <label className={labelCls}>Alasan Lembur <span className="text-red-400 normal-case">*</span></label>
            <div className="space-y-2">
              {REASON_OPTIONS.map((opt) => { const isSel = reasonType === opt.value; return (<button key={opt.value} type="button" onClick={() => { setReasonType(opt.value); if (opt.value !== "Lainnya") setReasonCustom(""); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border text-left transition-all active:scale-[0.99] ${isSel ? "bg-gray-900 border-gray-900 shadow-sm" : "bg-white border-gray-200 hover:border-gray-300 hover:bg-gray-50"}`}><span className="text-lg flex-shrink-0">{opt.icon}</span><div className="min-w-0"><p className={`text-sm font-bold leading-tight ${isSel ? "text-white" : "text-gray-800"}`}>{opt.value}</p><p className={`text-[10px] mt-0.5 leading-tight ${isSel ? "text-gray-300" : "text-gray-400"}`}>{opt.desc}</p></div>{isSel && <span className="ml-auto flex-shrink-0 w-5 h-5 rounded-full bg-white/20 flex items-center justify-center text-white text-xs font-black">✓</span>}</button>); })}
            </div>
          </div>
          {isLainnya && (<div><label className={labelCls}>Jelaskan Alasan <span className="text-red-400 normal-case">*</span></label><input type="text" value={reasonCustom} onChange={(e) => setReasonCustom(e.target.value)} placeholder="Tuliskan alasan spesifik..." className={inputCls} autoFocus /></div>)}
          <div><label className={labelCls}>Rincian Pekerjaan</label><textarea value={workDesc} onChange={(e) => setWorkDesc(e.target.value)} rows={3} placeholder="Jelaskan pekerjaan yang dikerjakan saat lembur..." className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 transition-all resize-none placeholder:text-gray-300" /></div>
          <div><label className={labelCls}>Status</label><select value={status} onChange={(e) => setStatus(e.target.value as OvertimeRequest["status"])} className={inputCls + " cursor-pointer"}>{STATUS_OPTIONS.map((s) => (<option key={s} value={s}>{STATUS_CONFIG[s]?.label ?? s}</option>))}</select></div>
          <div>
            <label className={labelCls}>Foto Bukti <span className="normal-case tracking-normal text-gray-400 font-normal">(opsional)</span></label>
            {photoStep === "preview" && photoPreview ? (<div><img src={photoPreview} alt="Preview" className="w-full h-44 object-cover rounded-2xl border border-gray-100 shadow-md mb-3" /><button onClick={() => { setPhotoFile(null); setPhotoPreview(null); setPhotoStep("idle"); }} className="text-xs font-bold text-gray-400 hover:text-gray-700 transition-colors">↺ Hapus Foto</button></div>)
            : photoStep === "camera" ? (<CameraCapture onCapture={(file, url) => { setPhotoFile(file); setPhotoPreview(url); setPhotoStep("preview"); setError(""); }} onCancel={() => setPhotoStep("idle")} />)
            : (<button onClick={() => setPhotoStep("camera")} className="w-full flex items-center justify-center gap-3 border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center hover:border-violet-300 hover:bg-violet-50/30 transition-all active:scale-[0.99] group"><span className="text-2xl group-hover:scale-110 transition-transform">📷</span><div className="text-left"><p className="text-sm font-bold text-gray-700">Buka Kamera</p><p className="text-xs text-gray-400 mt-0.5">Ambil foto bukti langsung</p></div></button>)}
          </div>
        </div>
      ...
      <div className="px-6 py-5 space-y-4">
        ...
        <div className="bg-red-50 border border-red-100 rounded-2xl p-4 space-y-2">
          ...
          <div className="space-y-1.5 text-sm">
            <div className="flex justify-between"><span className="text-gray-400">Karyawan</span><span className="font-bold text-gray-800">{overtime.users?.name ?? "—"}</span></div>
            <div className="flex justify-between"><span className="text-gray-400">Tanggal</span><span className="font-bold text-gray-800">{new Date(overtime.request_date).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}</span></div>
            <div className="flex justify-between"><span className="text-gray-400">Waktu</span><span className="font-bold font-mono text-gray-800">{formatTime(overtime.scheduled_start)} – {formatTime(overtime.scheduled_end)}</span></div>
            <div className="flex justify-between items-center"><span className="text-gray-400">Status</span><StatusBadge status={overtime.status} /></div>
            {showPay && overtime.total_pay != null && (<div className="flex justify-between"><span className="text-gray-400">Total Bayaran</span><span className="font-bold text-gray-800">{formatRupiah(overtime.total_pay)}</span></div>)}
          </div>
        </div>
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3">
          <span className="text-base flex-shrink-0">⚠️</span>
          <p className="text-xs text-amber-700 font-medium leading-relaxed">Tindakan ini <strong>tidak dapat dibatalkan</strong>. Data lembur dan foto bukti akan dihapus permanen.</p>
        </div>
      </div>
      <ModalFooter>
        <button onClick={onClose} className={secondaryBtn}>Batal</button>
        <button onClick={handleDelete} disabled={deleting} className="flex-1 h-11 bg-red-600 hover:bg-red-700 text-white rounded-xl text-sm font-bold transition-all active:scale-[0.98] flex items-center justify-center gap-2 shadow-sm disabled:opacity-40 disabled:cursor-not-allowed">{deleting ? <Spinner /> : "🗑️ Hapus Permanen"}</button>
      </ModalFooter>
    </ModalWrapper>
  );
}
...
function ActionDropdown({ items }: { items: ActionItem[] }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-all border border-gray-200 text-sm font-bold leading-none"
        title="Aksi lainnya"
      >⋯</button>
      {open && (
        <div className="absolute right-0 top-full mt-1 z-30 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden min-w-[152px]" style={{ animation: "modalUp 0.15s ease" }}>
          {items.map((item, i) => (
            <button key={i} onClick={() => { item.onClick(); setOpen(false); }}
              className={`w-full flex items-center gap-2.5 px-3.5 py-2.5 text-xs font-bold text-left transition-colors ${variantCls[item.variant ?? "default"]}`}>
...
// ── OvertimeTable ─────────────────────────────────────────────────────────────
function OvertimeTable({ rows, showDateCol, userCanViewPay, renderActions }: {
  rows: OvertimeRequest[];
  showDateCol: boolean;
  userCanViewPay: boolean;
  renderActions: (o: OvertimeRequest) => React.ReactNode;
}) {
  const EmployeeCell = ({ o }: { o: OvertimeRequest }) => (
    <div className="flex items-center gap-2.5">
      <div className={`w-8 h-8 rounded-xl bg-gradient-to-br ${avatarColor(o.users?.name || "")} flex items-center justify-center text-white text-[9px] font-black flex-shrink-0 shadow-sm`}>
      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b border-gray-100 bg-gray-50/80">
            {headers.map((h) => (
              <th key={h.key} className={`px-4 py-3 text-[9px] font-black text-gray-400 uppercase tracking-widest whitespace-nowrap ${h.align}`}>
                {h.label}
              </th>
            ))}
          </tr>
        ...
        <tbody className="divide-y divide-gray-50/80">
          {rows.map((o) => (
            <tr key={o.id} className="hover:bg-gray-50/60 transition-colors">
              <td className="px-4 py-3"><EmployeeCell o={o} /></td>
              {showDateCol && (
                <td className="px-3 py-3 whitespace-nowrap">
                  <span className="font-mono font-bold text-gray-600 text-[11px]">
                    {new Date(o.request_date).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" })}
                  </span>
                </td>
              )}
              <td className="px-3 py-3 whitespace-nowrap">
                <span className="font-mono font-semibold text-gray-700 text-[11px]">{formatTime(o.scheduled_start)} – {formatTime(o.scheduled_end)}</span>
              </td>
              <td className="px-3 py-3" style={{ maxWidth: 140 }}>
                <span className="text-gray-500 text-[11px] line-clamp-2 leading-relaxed" title={o.reason || ""}>{o.reason || "—"}</span>
              </td>
              <td className="px-3 py-3" style={{ maxWidth: 160 }}>
                <span className="text-gray-500 text-[11px] line-clamp-2 leading-relaxed" title={o.work_description || ""}>{o.work_description || "—"}</span>
              </td>
              <td className="px-3 py-3"><StatusBadge status={o.status} /></td>
              {userCanViewPay && (
                <td className="px-3 py-3 text-right whitespace-nowrap">
                  {o.total_pay != null
                    ? <span className="font-bold text-gray-800 font-mono text-[11px]">{formatRupiah(o.total_pay)}</span>
                    : <span className="text-gray-300 text-[11px]">—</span>
                  }
                </td>
              )}
              <td className="px-3 py-3">{renderActions(o)}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
export default function OvertimePage() {
  ...
  useEffect(() => {
    ...
    const parseTimestamp = (iso: string | null): number => {
      if (!iso) return 0;
      try { const d = new Date(iso); const t = d.getTime(); return isNaN(t) ? 0 : t; }
      catch { return 0; }
    };
    const checkAutoComplete = setInterval(() => {
      ...
      overtimes.forEach((o) => {
        ...
        if (scheduledEndTime === 0 || scheduledEndTime > now) return;
  ...
  const thisMonthOvertimes = overtimes.filter((o) =>
    toWIBDateKey(o.request_date).startsWith(`${calendarMonth.year}-${pad2(calendarMonth.month + 1)}`)
  );

  const byDate = useMemo(() => {
    const m: Record<string, OvertimeRequest[]> = {};
    thisMonthOvertimes.forEach((o) => { const k = toWIBDateKey(o.request_date); if (!m[k]) m[k] = []; m[k].push(o); });
    return m;
  }, [thisMonthOvertimes]);
  ...
  const renderActions = (o: OvertimeRequest) => {
    const primaryAction = (() => {
      if (currentUser?.id === o.user_id && o.status === "NEED_PROOF")
        return <button onClick={() => setCompleteData(o)} className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1.5 rounded-lg bg-orange-500 hover:bg-orange-600 text-white transition-all active:scale-95 whitespace-nowrap animate-pulse">📸 Upload Foto</button>;
      if (currentUser?.id === o.user_id && o.status === "ONGOING" && !o.actual_end)
        return <button onClick={() => setCompleteData(o)} className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white transition-all active:scale-95 whitespace-nowrap">🏁 Selesai</button>;
      if (currentUser?.id === o.user_id && o.status === "COMPLETED" && !o.proof_photo_url)
        return <button onClick={() => setCompleteData({ ...o, auto_completed: true })} className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-white transition-all active:scale-95 whitespace-nowrap">📸 Upload</button>;
      if (isAdminRole(currentUser?.role) && o.status === "PENDING")
        return <button onClick={() => setApproveData(o)} className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1.5 rounded-lg bg-violet-600 hover:bg-violet-700 text-white transition-all active:scale-95 whitespace-nowrap">✅ Setujui</button>;
      return null;
    })();

    const secondaryItems: ActionItem[] = [];
    if (o.proof_photo_url && (canApproveRole(currentUser?.role) || currentUser?.id === o.user_id))
      secondaryItems.push({ label: "Lihat Foto", icon: "👁️", onClick: () => setProofPhotoData(o) });
    if (canSetPay(currentUser?.role) && (o.status === "COMPLETED" || o.status === "NEED_PROOF"))
      secondaryItems.push({ label: o.rate_per_hour ? "Edit Bayaran" : "Set Bayaran", icon: "💰", onClick: () => setSetPayData(o), variant: "success" });

    const adminItems: ActionItem[] = [];
    if (isAdminRole(currentUser?.role)) {
      adminItems.push({ label: "Edit",  icon: "✏️", onClick: () => setEditData(o) });
      adminItems.push({ label: "Hapus", icon: "🗑️", onClick: () => setDeleteData(o), variant: "danger" });
    }

    const allDropdownItems: ActionItem[] = [...secondaryItems, ...adminItems];
    const hasPreDivider = primaryAction !== null || secondaryItems.length > 0;
      ...
      <div className="flex items-center justify-center gap-1">
        ...
        {secondaryItems.map((item, i) => (
          <button key={i} onClick={item.onClick}
            className={`hidden sm:inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1.5 rounded-lg transition-all active:scale-95 whitespace-nowrap ${item.variant === "success" ? "bg-emerald-600 hover:bg-emerald-700 text-white" : "bg-blue-600 hover:bg-blue-700 text-white"}`}>
            {item.icon} {item.label}
          </button>
        ))}
        {adminItems.length > 0 && hasPreDivider && <div className="hidden sm:block w-px h-4 bg-gray-200 mx-0.5 flex-shrink-0" />}
        {adminItems.map((item, i) => (
          <button key={i} onClick={item.onClick}
            className={`hidden sm:inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1.5 rounded-lg transition-all active:scale-95 whitespace-nowrap border ${item.variant === "danger" ? "bg-red-50 hover:bg-red-100 text-red-600 border-red-100" : "bg-gray-100 hover:bg-gray-200 text-gray-700 border-gray-200"}`}>
            {item.icon}{item.label !== "Hapus" ? ` ${item.label}` : ""}
          </button>
        ))}
        {allDropdownItems.length > 0 && <div className="sm:hidden"><ActionDropdown items={allDropdownItems} /></div>}
      </div>
    );
  };
  ...
  return (
    <DashboardLayout>
      <div className="min-h-screen bg-[#F7F7F8]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 space-y-5 sm:space-y-6">
          ...
          {/* ── STAT CARDS — 4 cards, 2×2 mobile, 4 kolom desktop ─────────── */}
            ...
            {statCards.map((c) => (
              <div key={c.label}
                ...
                <div className="p-4 sm:p-5 flex items-center gap-3.5">
                  <div className={`w-10 h-10 rounded-2xl bg-gradient-to-br ${c.accent} flex items-center justify-center text-lg flex-shrink-0 ring-4 ${c.ring}`}>
                    {c.icon}
                  </div>
                  <div className="min-w-0">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-0.5">Kalender Lembur</p>
                  <p className="font-black text-gray-900 text-base leading-tight">{MONTH_NAMES[calendarMonth.month]} {calendarMonth.year}</p>
                </div>
              </div>
              <div className="flex items-center gap-1">
                <button onClick={() => setCalendarMonth((m) => ({ month: m.month === 0 ? 11 : m.month - 1, year: m.month === 0 ? m.year - 1 : m.year }))}
                  className="w-8 h-8 rounded-xl hover:bg-gray-100 flex items-center justify-center text-gray-500 hover:text-gray-800 transition-all active:scale-95 text-base font-bold">‹</button>
                <button onClick={() => setCalendarMonth({ year: new Date().getFullYear(), month: new Date().getMonth() })}
                  className="h-8 px-3 rounded-xl hover:bg-gray-100 text-xs font-bold text-gray-500 hover:text-gray-800 transition-all">Hari ini</button>
            ...
            <div className="p-4 sm:p-5 overflow-x-auto">
              <div className="grid grid-cols-7 gap-1 mb-1 min-w-[300px]">
                {DAY_NAMES.map((d, i) => (
                  <div key={d} className={`text-center text-[9px] font-black py-1.5 uppercase tracking-wider ${i === 0 ? "text-red-400" : "text-gray-400"}`}>{d}</div>
                ))}
              </div>
                ...
                {calDays.map((day, idx) => {
                  ...
                  return (
                    <button key={day} onClick={() => setSelectedDate(selectedDate === dk ? null : dk)}
                      className={`relative flex flex-col items-center justify-start pt-2 pb-1.5 rounded-xl min-h-[60px] sm:min-h-[72px] transition-all duration-150 border ${isSel ? "bg-gray-900 border-gray-800 shadow-lg" : total > 0 ? "bg-violet-50 border-violet-100 hover:bg-violet-100 hover:border-violet-200" : "bg-transparent border-gray-100 hover:bg-gray-50 hover:border-gray-200"}`}>
                      <span className={`text-sm font-black mb-1 transition-colors ${isSel ? "text-white" : isToday ? "text-violet-600" : isSun ? "text-red-400" : "text-gray-700"}`}>{day}</span>
                      {isToday && !isSel && <span className="absolute top-1 right-1 w-1.5 h-1.5 rounded-full bg-violet-500" />}
                      {total > 0 && (
                        <span className={`text-[9px] font-black rounded-full px-1.5 py-0.5 leading-tight ${isSel ? "bg-white/20 text-white" : "bg-violet-500 text-white"}`}>{total}</span>
                      )}
                    </button>
                  );
                })}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              {selectedOvertimes.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 gap-2">
                  ...
                  <p className="text-sm font-bold text-gray-400">Tidak ada lemburan pada tanggal ini</p>
                </div>
              ) : (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-[11px] font-bold text-gray-400 bg-gray-100 px-3 py-1 rounded-full tabular-nums">{filtered.length} data</span>
                </div>
                  ...
                  {["Semua", ...statuses].map((s) => {
                    ...
                    return (
                      <button key={s} onClick={() => setFilterStatus(s)}
                        className={`flex-shrink-0 px-3 py-1.5 rounded-xl text-[11px] font-bold transition-all duration-150 border ${isActive ? "bg-gray-900 text-white border-gray-900 shadow-sm" : "bg-white text-gray-500 border-gray-200 hover:border-gray-300 hover:text-gray-700 hover:bg-gray-50"}`}>
              ...
              {loading ? (
                <div className="p-5 space-y-2">
                  {Array(5).fill(0).map((_, i) => (
                    <div key={i} className="h-12 bg-gray-50 rounded-xl animate-pulse" style={{ opacity: 1 - i * 0.15 }} />
                  ))}
                </div>
              ) : filtered.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 gap-2">
                  <div className="w-14 h-14 rounded-2xl bg-gray-100 flex items-center justify-center text-3xl">📭</div>
                  <p className="text-sm font-bold text-gray-400">Belum ada data lemburan</p>
                  <p className="text-xs text-gray-300">Coba ubah filter status di atas</p>
                </div>
              ) : (
