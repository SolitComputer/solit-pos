  ...
  if (!row) {
    ...
    if (!seed) {
      return {
        token: null,
        igUserId: null,
        error: "Instagram belum terhubung — isi IG_ACCESS_TOKEN lalu jalankan keepalive",
      };
    }

    const out = await exchangeLongLived(seed);
    if (out.token === null) {
      // exchange gagal (mis. FB_APP_ID belum di-set) → tetap coba pakai token mentahnya
      return { token: seed, igUserId, error: null };
    }
...
/** Dipanggil cron harian — memperpanjang token sebelum kedaluwarsa. */
export async function keepAliveIg(): Promise<{ ok: boolean; error: string | null }> {
  const auth = await getValidIgToken();
  return auth.token === null ? { ok: false, error: auth.error } : { ok: true, error: null };
}
...
export async function getIgHealth(): Promise<IgHealth> {
  ...
  if (!row) {
    return {
      ...
      accountName: null,
      daysLeft: null,
      level: process.env.IG_ACCESS_TOKEN ? "WARN" : "DEAD",
      error: process.env.IG_ACCESS_TOKEN
        ? "Token IG masih di env — jalankan keepalive agar tersimpan & auto-refresh"
