...
export type CameraCaptureProps = {
  onCapture: (f: File, url: string) => void;
  onCancel: () => void;
  // ✅ opsional — dipakai OvertimeTable.tsx buat nampilin nama karyawan
  // sebagai baris ke-2 watermark. Modal lain boleh gak isi ini.
  watermarkSubTag?: string;
};

// ✅ Komponen kamera realtime (live preview) untuk semua upload foto bukti
// lembur — TANPA opsi galeri/file. Dipakai oleh page.tsx (ManualOvertimeModal,
// EditOvertimeModal, CompleteModal), OvertimeTable.tsx (OvertimeQuickDetailForm,
// OvertimeProofUploadForm), dan OvertimeFillDetailModal.tsx.
export function CameraCapture({ onCapture, onCancel, watermarkSubTag }: CameraCaptureProps) {
  ...
  const capture = async () => {
    ...
    setProcessing(true);
    try {
      const res = await addTimestampWatermark(c, { tag: "SOLIT POS • BUKTI LEMBUR", subTag: watermarkSubTag });
      ...
      {error ? (
        <div className="rounded-xl bg-red-50 border border-red-200 p-5 text-center space-y-3">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-xl text-xs font-semibold text-gray-700 hover:bg-gray-50 transition-all shadow-sm"
          >
