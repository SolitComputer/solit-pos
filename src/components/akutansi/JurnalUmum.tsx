...
export default function JurnalUmum({ period }: { period: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [jRes, pRes] = await Promise.all([
        fetch(`/api/akuntansi/jurnal?period=${period}`),
        fetch(`/api/akuntansi/jurnal/pending?period=${period}`),
      ]);
  ...
  const confirmItems = async (items: PendingDraft[]) => {
    ...
    setBusy(true);
    try {
      const res = await fetch("/api/akuntansi/jurnal/confirm", {
        method: "POST",
  ...
  const handleDelete = async (entry: JournalEntry) => {
    ...
    setBusy(true);
    try {
      const res = await fetch(`/api/akuntansi/jurnal/${entry.id}`, { method: "DELETE" });
      const json = await res.json();
...
function EntryFormModal({
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      const url = isEdit ? `/api/akuntansi/jurnal/${entry!.id}` : "/api/akuntansi/jurnal";
              ...
              {lines.map((l, i) => (
                <div key={i} className="flex gap-2 items-center">
                  <select
                    ...
                    className={`h-9 w-24 border rounded-lg px-2 text-xs font-bold ${
                      l.side === "DEBIT"
...
function AuditLogModal({ entry, onClose }: { entry: JournalEntry; onClose: () => void }) {
  ...
  useEffect(() => {
    fetch(`/api/akuntansi/jurnal/${entry.id}/logs`)
      .then((r) => r.json())
