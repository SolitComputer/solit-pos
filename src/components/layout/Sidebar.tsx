...
const Icons = {
  ...
  log: (<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="5" y="4" width="14" height="17" rx="2" /><path d="M9 4V3a1 1 0 011-1h4a1 1 0 011 1v1" /><path d="M9 13l2 2 4-4" /></svg>),
  ...
  patchNotes: (<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>),
...
// Versi tanpa "Selesai Disiapkan" — khusus Kepala Zenith, Kepala Sales, Kepala Onpoint,
// Kepala Sotech. Mereka tetap bisa lihat Dashboard Barang, tapi TIDAK bisa terima/cek unit
// (itu action khusus role Penyedia Barang).
const PREPARATION_PENYEDIA_VIEW_MENU: MenuGroup = {
  label: "Penyiapan Barang",
  items: [ITEM_RIWAYAT_PENYEDIA],
};
...
const CREW_SALES_INVENTARIS: MenuGroup = {
  label: "Inventaris",
  items: [ITEM_LAPTOP_SIAP_JUAL, { name: "Garansi", href: "/dashboard/warranty", icon: Icons.garansi }],
};
...
const ROLE_MENUS: Record<UserRole, MenuGroup[]> = {
  ...
  KEPALA_ZENITH: [
    ...
    PREPARATION_SALES_MENU, PREPARATION_PENYEDIA_VIEW_MENU, PREPARATION_SALES_DELIVERY_MENU,
  ],
  ...
  CREW_SALES: [
    ...SALES_OVERVIEW([ITEM_USERS]), CREW_SALES_INVENTARIS, SALES_TRANSAKSI,
    PREPARATION_SALES_MENU, PREPARATION_PENYEDIA_MENU, PREPARATION_SALES_DELIVERY_MENU,
  ],
  ...
  PENGANTARAN: [
    ...SALES_OVERVIEW([ITEM_USERS]), CREW_SALES_INVENTARIS, PENGANTARAN_TRANSAKSI, PREPARATION_PENGANTARAN_MENU,
  ],
...
(Object.keys(ROLE_MENUS) as UserRole[]).forEach((role) => {
  ...
  ROLE_MENUS[role] = ROLE_MENUS[role].map((g) => {
    ...
    hasInventaris = true;
    if (g.items.some((it) => it.href === ITEM_AUDIT_OUTFLOW.href)) return g;
    return { label: g.label, items: [...g.items, ITEM_AUDIT_OUTFLOW] };
  });
  if (!hasInventaris) {
