          ...
          {onlineUsers.map(p => {
            ...
            return (
              <div
                ...
                onClick={() => { if (!isSelf) onChatOpen({ id: p.user_id, name: p.user_name, role: p.user_role, profile_photo_url: p.profile_photo_url }); }}
              >
                {!isSelf && <ChatHintTooltip />}
                <div className="relative flex-shrink-0">
                  <div className="w-6 h-6 rounded-full bg-gradient-to-br from-[#1a1a2e] to-[#16213e] flex items-center justify-center text-white text-[8px] font-black overflow-hidden">
                    {p.profile_photo_url
                      ? <img src={p.profile_photo_url} alt={p.user_name} className="w-full h-full object-cover" />
        ...
        <div className="divide-y divide-gray-100 overflow-y-auto max-h-80">
          {filtered.map(p => {
            ...
            return (
              ...
              >
                <div className="relative flex-shrink-0">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-[9px] font-black shadow-sm overflow-hidden ${p.is_online ? "bg-gradient-to-br from-[#1a1a2e] to-[#16213e]" : "bg-gray-400"
                    }`}>
                    {p.profile_photo_url
                      ? <img src={p.profile_photo_url} alt={p.user_name} className="w-full h-full object-cover" />
                      : initials(p.user_name)}
...
export function OnlineUsersPanel() {
  ...
  useEffect(() => {
    ...
    const ticker = setInterval(() => setPresence(prev => recalculate(prev)), 5_000);
