...
export function SellerReminderNotifier({ userId }: { userId: string | null | undefined }) {
  ...
  // ── Render bubble via portal ke document.body ──
  // Ini memastikan position:fixed benar-benar relatif ke viewport asli,
  // bukan ke ancestor manapun (mis. wrapper dengan transform/max-width)
  // yang bisa "mengunci" area geser sehingga tidak bisa mencapai sisi kanan layar.
  ...
  const hasAnything = unreadCount > 0 || dueCount > 0;

  // Kalau semua sudah beres (tidak ada reminder & tidak ada yang overdue),
  // tutup otomatis panel yang mungkin sedang terbuka.
  useEffect(() => {
  ...
  if (!hasAnything) return null; // ← tidak ada follow-up wajib sama sekali → jangan tampil
  ...
  const content = (
    <>
      ...
      {/* ── Bubble mengambang — bisa digeser bebas ke seluruh layar, snap balik ke tepi saat dilepas ── */}
      <button
        ref={bubbleRef}
