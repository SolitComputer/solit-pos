...
// Warna avatar konsisten per role
...
function MessageBubble({ msg, isMine, isAdmin, onReply, onDelete, onScrollToReply }: BubbleProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-col max-w-[72%] ${isMine ? "items-end" : "items-start"}`}>
        {/* Nama pengirim (hanya untuk non-mine) */}
        {!isMine && (
          <span className="text-[10px] font-semibold mb-0.5 px-1" style={{ color: getAvatarColor(msg.sender_role) }}>
      ...
      {/* Action tombol hover (quick reply) */}
...
export function GroupChatPanel({ currentUser, onClose }: GroupChatPanelProps) {
  ...
  // ── Auto scroll ke bawah saat messages pertama kali load ─────────────────────
  useEffect(() => {
  ...
  // ── Scroll ke bawah saat pesan baru masuk (jika tidak scroll up) ──────────────
  ...
  useEffect(() => {
    if (messages.length > prevMsgCount.current) {
      ...
      if (isMyMsg || !isScrolledUp) {
        ...
        // Hitung unread dari orang lain
        const newUnreadCount = messages.slice(prevMsgCount.current)
  ...
  const handleScroll = useCallback(() => {
    ...
    // Load more ketika scroll ke atas
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          setMessages(prev =>
            prev.map(m => m.id === updatedMsg.id ? { ...m, is_deleted: updatedMsg.is_deleted } : m)
          );
        }
      )
      .subscribe();
  ...
  const groupedMessages = messages.reduce<Array<{ dateKey: string; dateLabel: string; msgs: GroupMessage[] }>>(
    (acc, msg) => {
  ...
  return (
    ...
    >
      ...
      >
            ...
            groupedMessages.map(group => (
              <Fragment key={group.dateKey}>
                ...
                {group.msgs.map((msg, idx) => {
                  ...
                  // Spacer kecil antara pesan dari user berbeda
        ...
        {/* ── Input area ── */}
        <div
          className="flex-shrink-0 flex items-end gap-3 px-4 py-3 border-t border-gray-100 bg-white"
        >
          {/* Avatar pengirim */}
          <div className="flex-shrink-0 self-end mb-1">
          ...
          >
            {sending ? (
              ...
              <svg className="w-4 h-4" fill="none" stroke={input.trim() ? "white" : "#9ca3af"} viewBox="0 0 24 24">
