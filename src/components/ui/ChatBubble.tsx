                    ...
                    <div className="max-w-[210px] px-3 py-2"
                        ...
                        {hasAttachment && (
                            <div className={hasContent ? "mb-1.5" : ""}>
                                <AttachmentDisplay url={msg.attachment_url!} type={msg.attachment_type!}
                                    name={msg.attachment_name} size={msg.attachment_size} isMine={isMine} />
                            </div>
                        )}
...
export function ChatPanel({ currentUser, targetUser, isMinimized, onToggleMinimize, onClose, unread }: ChatPanelProps) {
    ...
    const sendAttachment = async () => {
        ...
        try {
            ...
            const msg = await fetch("/api/messages", {
                method: "POST", headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ receiver_id: targetUser.id, content: caption, attachment_url: upData.url, attachment_type: upData.type, attachment_name: upData.name, attachment_size: upData.size }),
            });
    ...
    const handleKey = (e: React.KeyboardEvent) => {
        if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); preview ? sendAttachment() : send(); }
    };

    const isUploading = sending || uploading;
    const canSend = preview ? !uploading : (!!input.trim() && !sending);

    // ── Minimized state — hanya header kecil di bar ──────────────────────────
    if (isMinimized) return null; // Ditangani di ChatBar sebagai tab
...
// ─── ChatManager (bottom bar + panels) ───────────────────────────────────────
//
// Desain: strip tipis 36px di bawah layar.
// Tab kecil tiap chat aktif. Klik tab → expand panel ke atas.
// Grup Chat button di kiri strip.
//
...
export function ChatManager({ currentUser, activeChats, onClose }: ChatManagerProps) {
    ...
    // expandedId: chat mana yang sedang terbuka ke atas. null = semua minimized.
    const [expandedId, setExpandedId] = useState<string | null>(null);
    // unread per user
    const [unreadMap, setUnreadMap] = useState<Record<string, number>>({});

    // Auto-expand chat terbaru ketika baru ditambah
    ...
    // Tidak render apa-apa jika tidak ada chat aktif dan grup chat tidak buka
    const hasAnything = activeChats.length > 0 || openGroupChat;

    return (
        <>
            {/* ── Bottom strip: sangat tipis, selalu muncul di dashboard ─── */}
            <div
                ...
                style={{
                    ...
                    paddingLeft: 260, // offset sidebar lebar ≈ 224px + sedikit margin
                    paddingRight: 12,
                    gap: 4,
                }}
            >
                {/* Grup Chat button — selalu tampil */}
                <button
            ...
            {/* ── Chat panels (expanded) — muncul tepat di atas bar ─────────── */}
            {hasAnything && (
                <div className="fixed z-[9991] flex items-end gap-2"
                    style={{
                        bottom: 38, // tepat di atas bar 38px
                        right: 16,
                    }}>
                    {/* Grup Chat Panel */}
                    {openGroupChat && (
...
// ─── Backward-compat export (dipakai di beberapa tempat lama) ─────────────────
