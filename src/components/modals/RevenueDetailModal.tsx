...
export function RevenueDetailModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const maxWeeklyRevenue = data ? Math.max(...data.weekly.map((w) => w.revenue), 1) : 1;
  ...
  return (
    <>
      <style>{`
        ...
        .rdm-scrollbar::-webkit-scrollbar {
          width: 4px;
        }
        .rdm-scrollbar::-webkit-scrollbar-track {
          background: transparent;
        }
        .rdm-scrollbar::-webkit-scrollbar-thumb {
        ...
        .rdm-stat-glow-green {
          box-shadow: 0 0 30px rgba(16,185,129,0.12), inset 0 1px 0 rgba(255,255,255,0.06);
        }
        .rdm-stat-glow-amber {
          box-shadow: 0 0 30px rgba(245,158,11,0.12), inset 0 1px 0 rgba(255,255,255,0.06);
        }
        .rdm-stat-glow-blue {
          box-shadow: 0 0 30px rgba(99,102,241,0.12), inset 0 1px 0 rgba(255,255,255,0.06);
        }
          ...
          <div className="flex items-center justify-between px-5 sm:px-6 py-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs" style={{ color: "rgba(255,255,255,0.4)" }}>Rincian penjualan berkala</p>
              </div>
            </div>
            <button
              onClick={onClose}
            ...
            {isLoading ? (
              <div className="p-5 space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="rdm-shimmer rounded-xl h-20" />
                  ))}
                </div>
                {/* Shimmer rows */}
                <div className="space-y-2 pt-3">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <div key={i} className="rdm-shimmer rounded-xl h-14" />
                  ))}
                </div>
              </div>
            ) : data ? (
              <div className="p-5 space-y-5">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {/* Hari ini */}
                  <div className="rdm-card rdm-stat-glow-green rounded-xl p-3.5">
                    <div className="w-6 h-6 rounded-lg flex items-center justify-center mb-2" style={{ background: "rgba(16,185,129,0.2)" }}>
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#10B981" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                    </div>
                    <p className="text-[9px] font-semibold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.4)" }}>Hari Ini</p>
                  ...
                  {/* Bulan ini */}
                  <div className="rdm-card rdm-stat-glow-amber rounded-xl p-3.5">
                    ...
                    <p className="text-sm font-bold mt-0.5" style={{ color: "#F1F5F9" }}>{fmtShort(data.monthly.revenue)}</p>
                    <p className="text-[9px] mt-1" style={{ color: "rgba(255,255,255,0.35)" }}>{data.monthly.count} transaksi</p>
                  </div>

                  {/* Profit */}
                  <div className="rdm-card rdm-stat-glow-blue rounded-xl p-3.5">
                    <div className="w-6 h-6 rounded-lg flex items-center justify-center mb-2" style={{ background: "rgba(99,102,241,0.2)" }}>
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#818CF8" strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
                    </div>
                    <p className="text-[9px] font-semibold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.4)" }}>Profit</p>
                    <p className="text-sm font-bold mt-0.5" style={{ color: "#F1F5F9" }}>{fmtShort(data.today.profit)}</p>
                    <p className="text-[9px] mt-1" style={{ color: "rgba(255,255,255,0.35)" }}>hari ini</p>
                  </div>
                </div>
                ...
                <div className="flex gap-2 p-1 rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
                  {tabs.map(({ key, label }) => (
                    <button
                      ...
                      className={`flex-1 py-2 text-xs font-semibold rounded-lg transition-all duration-200 ${activeTab === key ? "rdm-tab-active" : "rdm-tab-inactive"}`}
                    >
                      {label}
                    </button>
                  ))}
                  ...
                  <div className="space-y-2">
                    {data.daily.length === 0 ? (
                      ...
                      data.daily.map((day, idx) => (
                        <div key={day.date} className="rdm-row rdm-card rounded-xl p-3.5 cursor-default">
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <div className="text-right flex-shrink-0">
                              ...
                              <span className="rdm-profit-badge inline-block text-[9px] font-semibold px-1.5 py-0.5 rounded-md mt-0.5" style={{ color: "#34D399" }}>
                                +{fmtShort(day.profit)}
                              </span>
                            </div>
                          </div>
                          <MiniBar value={day.revenue} max={maxDailyRevenue} color="linear-gradient(90deg, #10B981, #34D399)" />
                        </div>
                      ))
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    {data.weekly.length === 0 ? (
                      ...
                      data.weekly.map((week, idx) => (
                        <div key={week.weekStart} className="rdm-row rdm-card rounded-xl p-3.5 cursor-default">
                          <div className="flex items-center justify-between gap-3">
                            <div className="flex items-center gap-3 min-w-0">
                          ...
                          <MiniBar value={week.revenue} max={maxWeeklyRevenue} color="linear-gradient(90deg, #F59E0B, #FCD34D)" />
                        </div>
                      ))
                    )}
                  </div>
                ...
                {activeTab === "monthly" && (
                  <div className="space-y-3">
                    {/* Revenue */}
                    <div className="rdm-card rounded-xl p-4" style={{ border: "1px solid rgba(16,185,129,0.2)" }}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2.5">
                          <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "rgba(16,185,129,0.15)" }}>
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#10B981" strokeWidth="2.2"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg>
                          </div>
                          <div>
                    ...
                    {/* Profit */}
                    <div className="rdm-card rounded-xl p-4" style={{ border: "1px solid rgba(99,102,241,0.2)" }}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2.5">
                          <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "rgba(99,102,241,0.15)" }}>
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#818CF8" strokeWidth="2.2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
                          </div>
                          <div>
                        ...
                        <p className="text-lg font-bold" style={{ color: "#34D399" }}>{fmtRupiah(data.monthly.profit)}</p>
                      </div>
                    </div>

                    {/* Transactions */}
                    <div className="rdm-card rounded-xl p-4" style={{ border: "1px solid rgba(245,158,11,0.2)" }}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2.5">
                          <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "rgba(245,158,11,0.15)" }}>
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#F59E0B" strokeWidth="2.2"><path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2"/><rect x="9" y="3" width="6" height="4" rx="1"/><line x1="9" y1="12" x2="15" y2="12"/><line x1="9" y1="16" x2="13" y2="16"/></svg>
                          </div>
                          <div>
                      ...
                      <div className="rounded-xl p-4" style={{ background: "linear-gradient(135deg, rgba(16,185,129,0.08) 0%, rgba(99,102,241,0.08) 100%)", border: "1px solid rgba(255,255,255,0.06)" }}>
                        ...
                        <div className="w-full h-1.5 rounded-full" style={{ background: "rgba(255,255,255,0.08)" }}>
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, (data.monthly.profit / data.monthly.revenue) * 100)}%`,
