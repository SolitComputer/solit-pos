  ...
  const send = async () => {
    ...
    try {
      const res = await fetch("/api/contracts", {
        ...
        body: JSON.stringify({ user_id: user.id, contract_type: type, title, content, valid_from: validFrom, duration_months: durationMonths }),
      });
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select
                  value={durationMonths === null ? "PERMANENT" : String(durationMonths)}
                  onChange={(e) => setDurationMonths(e.target.value === "PERMANENT" ? null : parseInt(e.target.value))}
                  className="w-full h-10 border border-gray-200 rounded-xl px-3 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-violet-400/30"
                >
